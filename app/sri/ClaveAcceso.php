...
class ClaveAcceso
{
    ...
    public function llenarDatos($campos)
    {
        $fecha_emision = null;
        ...
        if (!is_null($campos)) {
            if (is_array($campos)) {
                ...
                if (isset($campos["fecha_emision"])) {
                    $fecha = str_replace('/', '-', $campos["fecha_emision"]);
        ...
        $this->ruc = str_pad(trim($ruc), 13, "0", STR_PAD_LEFT);
        $this->tipo_ambiente = $tipo_ambiente;
        $this->clave_contigencia='0908202201092160589500120010010000000';
        $this->generaClave();
        $this->generaClaveContingencia();
    }
    ...
    public function generaClaveContingencia()
    {
