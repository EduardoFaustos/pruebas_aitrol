...
class Inventario2
{
    ...
    public static function build_process($type = "", $id = "", $enterprise, $asiento_t = 0, $importaciones = 0)
    {
        ...
        if ($type == "C") {
            ...
            if ($empresa->inventario == 1) {
                ...
                $compras = Inventario2::compras($id, $enterprise, $asiento_t, $import = $importaciones);
               // dd($compras);
                return $compras;
            }
            ...
            if ($empresa->inventario == 1) {
                $ventas = Inventario2::ventas($id, $enterprise);
                return $ventas;
            }
        }
        return 'error';
    ...
    public static function compras($id = "", $enterprise, $asiento_t = 0, $importaciones = 0)
    {
        if ($id != "") {
            ...
            foreach ($groupProduct as $key => $value) {
                ...
                if (!is_null($producto)) {
                    $exist = Inventario2::verifyexist($producto->id, $enterprise);
                   // dd($exist);
                    if ($exist == 'N') {
                  ...
                  // return [$importaciones, "valor"];
                   if($importaciones == 0){
                        //$asiento = Inventario2::asiento_compras($id, $producto->id, $asiento_t);
                   }
                }
            }
        }
    ...
    public static function ventas($id = "", $enterprise)
    {
        if ($id != "") {
            ...
            foreach ($groupProduct as $key => $value) {
                ...
                $asientos_ventas = Inventario2::asiento_ventas($id, $producto->id, $cantidad);
            }
        } else {
            return 'error';
        }
    ...
    public static function anular_compras($id)
    {
        ...
        if (!is_null($movimiento)) {
            ...
            $recalcular = Inventario2::recalcular_compras($id_inventario);
        }
    }
    public static function anular_ventas($id)
    {
        ...
        if (!is_null($movimiento)) {
            ...
            $recalcular = Inventario2::recalcular_ventas($id_inventario);
        }
    }
