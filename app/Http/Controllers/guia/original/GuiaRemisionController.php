...
class GuiaRemisionController extends Controller
{
    ...
    public function ci_nombres_destinatario(Request $request)
    {
        ...
        $id_empresa  = $request->session()->get('id_empresa');
        $ventas = Ct_ventas::where('nro_comprobante', 'like', '%' . $nombres . '%')->where('id_empresa', $id_empresa)->get([
    ...
    public function save_update(Request $request)
    {
        ...
        $detalle = Ct_Guia_Remision_Detalle::where('id_cabecera_remision', $guia_cabecera->id)->delete(['id']);
        $id_empresa  = $request->session()->get('id_empresa');
        $ip_cliente = $_SERVER["REMOTE_ADDR"];
        $transportistas = Trasportista::find($request['nombre_ruc']);
        $id_sucursal = '';
