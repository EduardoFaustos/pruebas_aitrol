...
class VentasController extends Controller
{
    ...
    public function ordenes(Request $request)
    {
        ...
        if($request['id']!=null){
            ...
            if($request->fecha_desde==null){
                $ventas= $ventas->where('fecha','<',$request->fecha_hasta);
            }
            if($request->nombres_paciente!=null){
                $ventas= $ventas->where('nombres_paciente','LIKE','%'.$request->nombres_paciente.'%');
            }
            if($request->nombres_cliente!=null){
                $ventas= $ventas->where('nombres_cliente','LIKE','%'.$request->nombres_cliente.'%');
            }
        }
        $ventas= $ventas->orderby('id', 'desc')->paginate(10);

      
        return view('contable/ventas/ordenes', ['ventas' => $ventas, 'empresa' => $empresa,'request'=>$request]);
    }
    ...
    {
        $planilla = array();    $fact_venta = array();      $orden = array();   $detalles = array(); 
    ...
    public function obtenerPlanillasAgenda($id)
    {
        ...
        return view('contable/ventas/modal_detalle_planillas', ['id' => $id, 'paciente' => $paciente, 'orden' => $orden, 'hc'=>$hc, 'id_usuario' => $id_usuario, 'agenda'=>$agenda, 'hcp' => $hcp ]);
    }
    ...
    public function index_recibo(Request $request){
        ...
        if($request->id!=null){
            ...
            if($request->id_asiento!=null){
                $ventas= $ventas->where('id_asiento_cabecera',$request->id_asiento);
            }
