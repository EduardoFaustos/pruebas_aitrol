...
class ClientesRetencionesController extends Controller
{
    ...
    public function buscar(Request $request)
    {
        ...
        return view('contable.clientes_retenciones.index', ['request' => $request, 'empresa' => $empresa, 'retenciones' => $retenciones, 'clientes' => $clientes, 'searchingVals' => $constraints, 'tipo_comprobante' => $tipo_comprobante]);
    }
    ...
    private function doSearchingQuery($constraints, Request $request)
    {
        ...
        $query          = DB::table('ct_cliente_retencion as ct_c')->join('ct_clientes as c', 'c.identificacion', 'ct_c.id_cliente')
            ...
            ->select('ct_c.id', 'c.nombre', 'ct_c.descripcion', 'v.nro_comprobante', 'ct_c.id_cliente', 'ct_c.created_at', 'ct_c.fecha', 'ct_c.secuencia', 'ct_c.id_usuariocrea', 'ct_c.valor_fuente', 'ct_c.valor_iva', 'ct_c.estado');
    ...
    public function anular($id, Request $request)
    {
        ...
        //dd($retenciones);
        /*      
        $retenciones = DB::table('ct_cliente_retencion')
            ->join('ct_asientos_cabecera', 'ct_cliente_retencion.id_asiento_cabecera', '=', 'ct_asientos_cabecera.id')
            ->where ('ct_cliente_retencion.id','=',$id)
            ->select('ct_cliente_retencion.*','ct_asientos_cabecera.*')
            ->first();  */
        //dd($retenciones);
        $id_empresa = $request->session()->get('id_empresa');
        DB::beginTransaction();
