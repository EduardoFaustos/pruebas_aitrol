...
class Contable
{
    ...
    public static function recovery_price($id = "", $type = "")
    {
        if ($type == "C") { //COMPRAS
            ...
            $egreso = Contable::egreso($id);
            $egreso_masivo = Contable::egreso_masivo($id);
            $cruce = Contable::cruce($id);
            $cruce_cuentas = Contable::cruce_cuentas($id);
            $retenciones = Contable::retenciones($id);
            $debito_bancario = Contable::debito($id);
            $debito_acreedores = Contable::debito_acreedores($id);
            $credito_acreedores = Contable::credito_acreedores($id);
            ...
            $retenciones = Contable::retenciones_clientes($id);
            $ingreso = Contable::ingreso($id);
            $cruce = Contable::cruce_clientes($id);
            $cruce_cuenta = Contable::cruce_cuentas_clientes($id);
            $credito = Contable::credito_clientes($id);
            $debito_clientes = Contable::debito_clientes($id);
            $chequepost = Contable::chequepost($id);
            ...
            if ($type == 'O') {
                if ($module == 'EG') {
                    ...
                    if (isset($ventas->retenciones)) {
                        $rete = Ct_Cliente_Retencion::where('id_factura', $id)->first();
                        if (($rete->estado == 1)) {
                            $arraymodules['retencion'] = $rete->id;
                        } else {
                            $arraymodules['retencionA'] = $rete->id;
                        }
                    }
                    $ing = [];
                    ...
                    $sql = "SELECT
                        ...
                        FROM ct_asientos_detalle, ct_asientos_cabecera
                            ...
                            AND ct_asientos_cabecera.id_empresa=" . $id . "
                    ...
                    foreach ($asientos as $a) {
                        ...
                        $xl['modulo'] = Contable::recovery_by_asiento($a->id_asiento_cabecera);
                        $xl['debe'] = $a->debe_n;
                        $xl['haber'] = $a->haber_n;
                        $xl['fecha'] = $a->fecha;
                ...
                $message = Contable::fix_secuencia($module, $id);
                return $message;
            } else if ($type == 'R') {
                $message = Contable::recovery_price($id, $module);
                return $message;
            } else if ($type == 'S') {
                $message = Contable::recovery_by_asiento($id);
            } else if ($type == 'RE-C') {
                ...
                foreach ($compras as $c) {
                    $message = Contable::recovery_price($c->id, "C");
                }
                ...
                foreach ($compras as $c) {
                    $message = Contable::recovery_price($c->id, "V");
                }
                return $message;
            } else if ($type == 'CR') {
                ...
                //dd('aa');
                $a = Contable::fix_comprobante($module, $id);
                return $a;
            } else if ($type == 'FDATE') {
                $a = Contable::fixdate($id);
            } else if ($type == 'SUP') {
                $message = Contable::verify_price($id);
            }
        }
    ...
    public static function recovery_by_asiento($id)
    {
        $modulos = [];
        //compras
        $compras = Contable::compras($id);
        $ventas = Contable::ventas($id);
        $bancos = Contable::bancos($id);
        ...
        if ($orden != null) {
            if ($valor_contable == 0) {
                ...
                Ct_Proceso_Detalle::create([
                    ...
                    'id_usuariocrea'      => $idusuario,
                    'ip_creacion'         => $ip_cliente,
                    'secuencia'           => Contable::secuence_process('P', $orden->id_empresa),
                ]);
            }
        }
...
/*         foreach ($compras as $c) {
            $valor_factura = $c->total_final;
            /* $valor_asiento= Ct_Asientos_Detalle::where('id_asiento_cabecera',$c->id_asiento_cabecera)->sum('debe') - Ct_Asientos_Detalle::where('id_asiento_cabecera',$c->id_asiento_cabecera)->sum('haber');
            if($valor_asiento==0){
                $valor_asiento= Ct_Asientos_Detalle::where('id_asiento_cabecera',$c->id_asiento_cabecera)->sum('debe');
            } */
          /*   $valor_asiento = Ct_Asientos_Detalle::where('id_asiento_cabecera', $c->id_asiento_cabecera)->where('id_plan_cuenta', '2.01.03.01.01')->sum('debe');
            if ($valor_asiento == 0) {
                $valor_asiento = Ct_Asientos_Detalle::where('id_asiento_cabecera', $c->id_asiento_cabecera)->where('id_plan_cuenta', '2.01.03.01.01')->sum('haber');
            }
            //$valor_cabecera= Ct_Asientos_Cabecera::find($c->id_asiento_cabecera)->valor;
            if ($valor_factura != $valor_asiento) {
                $new['id'] = $c->id;
                $new['id_asiento'] = $c->id_asiento_cabecera;
                $new['valor_factura'] = $valor_factura;
                $new['valor_asiento'] = $valor_asiento;
                array_push($array_desiguales_cab, $new);
            } */
            /*   if($valor_factura!=$valor_asiento){
                $new['valor_factura']=$valor_factura;
                $new['valor_asiento']=$valor_asiento;
                array_push($array_desiguales_det,$new);
            } 

        }
       /*  $arrayFin['array_cab'] = $array_desiguales_cab; */
       /*  $arrayFin['array_det'] = $array_desiguales_det; */
        $egreso= Ct_Comprobante_Egreso::where('id_empresa',$id)->get();
        foreach($egreso as $e){
            $valor_compra=0;
            $valor_proveedor=0;
            $valor_compra= $e->detalles->sum('abono');
            /* foreach($e->asiento_cabecera->detalles as $det){
                w
                $valor_proveedor+=$det->
            } */
