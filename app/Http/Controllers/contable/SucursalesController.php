...
class SucursalesController extends Controller
{
    ...
    /************************************************
     ****LISTADO DE ESTABLECIMIENTOS (SUSCURSALES)****
    /************************************************/
    public function index(Request $request)
    {
        ...
        $sucur = Ct_Sucursales::where('estado', '=', 1)->where('id_empresa', $id_empresa)->orderby('id', 'desc')->paginate(5);
        return view('contable.sucursales.index', ['sucur' => $sucur, 'empresa' => $empresa]);
    }
    /*************************************************
    ...
    public function store(Request $request)
    {
       ...
       $idusuario  = Auth::user()->id;
        $id_empresa = $request->session()->get('id_empresa');
        $ciudad = Ct_Ciudad::where('estado', '1')
            ->where('nombre', $request['ciud'])
            ->first();
    ...
    /*************************************************/
    public function guardarCiudad(Request $req)
    {
        $arrayCiudad = [
            'nombre' => $req->ciud,
            'estado' => 1,
            'id_usuariocrea' => Auth::user()->id,
            'id_usuariomod' => Auth::user()->id,
            'ip_creacion' => $_SERVER["REMOTE_ADDR"],
            'ip_modificacion' => $_SERVER["REMOTE_ADDR"],
            'created_at' => date('Y-m-d H:i:s'),
            'updated_at' => date('Y-m-d H:i:s')
        ];
        $ciudades = Ct_Ciudad::all();
        $existe = 0;
        foreach ($ciudades as $row) {
            if (strtolower($row->nombre) == strtolower($req->ciud))
                $existe++;
        }
        if ($existe == 0) {
            Ct_Ciudad::insert($arrayCiudad);
            return  $req->ciud;
        }
    }
    public function editar($id, $id_empresa)
    {
        $sucursal = Ct_Sucursales::
            where('ct_sucursales.id', $id)
            ->first();
       $empresa  = Empresa::where('id', $id_empresa)->first();
        $empresas = Empresa::where('estado', '1')->get();
        $ciudad = Ct_Ciudad::where('estado', '1')->get();
    ...
    public function update(Request $request)
    {
        ...
        $id = $request['id_estab'];
        $establecimiento = Ct_Sucursales::where('id', $id)->first();
        $id_empresa = $request->session()->get('id_empresa');
        $ciudad = Ct_Ciudad::where('estado', '1')
            ->where('nombre', $request['ciud'])
            ->first();
    ...
    public function buscar(Request $request)
    {
        ...
        $sucur = $this->doSearchingQuery($constraints, $request);
        return view('contable.sucursales.index', ['request' => $request, 'sucur' => $sucur, 'searchingVals' => $constraints, 'empresa' => $empresa]);
    }
    /*************************************************
     ******************CONSULTA QUERY******************
    /*************************************************/
