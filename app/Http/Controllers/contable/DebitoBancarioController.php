...
class DebitoBancarioController extends Controller
{
    ...
    public function generardebito(Request $request)
    {
        ...
        if ($contador_ctv == 0) {
            ...
            $max_id = DB::table('ct_debito_bancario')->where('id_empresa', $id_empresa)->orderBy('id','DESC')->first();
        ...
        for ($i = 0; $i <= $request['contador']; $i++) {
            ...
            if ($request['abono' . $i] > 0) {
                if ($consulta_compra != null) {
                    ...
                    //Contable::pagofactura($consulta_compra->id,$id_db,'DB');
                }
            }
        ...
        if ($request['debito_favor']>0) {
            ...
            $numero_factura2=0;
            $contador_ctv2 = DB::table('ct_comprobante_egreso')->where('id_empresa', $id_empresa)->get()->count();


            // if ($contador_ctv2 == 0) {
            //     $num = '1';
            //     $numero_factura2 = str_pad($num, 10, "0", STR_PAD_LEFT);
            ...
            //     if(is_null($max_id)){
            //         $nu = 1;
            //         $numero_factura2 = str_pad($nu, 9, "0", STR_PAD_LEFT);
            //         DB::table('ct_comprobante_secuencia')->insert([
            //             'tipo'=>1,
            //             'empresa'=> $id_empresa,
            //             'secuencia'=> $numero_factura2,
            ...
            // }
           // $numero_factura2 = $this->getSecuencia(2);

            $id_plan_confg = LogConfig::busqueda('1.01.04.03.01');
            $plan_cuentas = Plan_Cuentas::find($id_plan_confg);

            $numero_factura2 =  LogAsiento::getSecuencia(2);

            $asiento_detalle = [
                'id_asiento_cabecera'           => $id_asiento_cabecera,
                //'id_plan_cuenta'                => '1.01.04.03', //cambiar a anticipo proveedores
                //'descripcion'                   => 'Anticipo a Proveedores',
                'id_plan_cuenta'                => $plan_cuentas->id, //cambiar a anticipo proveedores
            ...
            $input_comprobante = [
                ...
                'fecha_comprobante' => $request['fecha_asiento'],
                'secuencia'       => $numero_factura2,
                'id_empresa'      => $id_empresa,
                'id_proveedor'    => $id_proveedor,
    ...
    public function anulacion($id, Request $request)
    {
        ...
        if (!is_null($id)) {
            ...
            if (!is_null($registro)) {
                ...
                if (!is_null($registro)) {
                    $egreso = Ct_Comprobante_Egreso::where('id_asiento_cabecera', $registro->id_asiento)->where('estado', '<>', '0')->first();
                    if (!is_null($egreso)) {
                        $egreso->estado = 0;
                ...
                Log_Contable::create([
                    'tipo'           => 'Débitos Bancarios Acreedores',
