...
class DoceavoPasoController extends Controller
{
    ...
    public function doceavopaso($id_sol)
    {
        ...
        return view('hospital.emergencia.doceavopaso',['id_sol' => $id_sol, 'solicitud' => $solicitud]);
    }


    public function agregar_cie10_alta(Request $request)
    {
        //return $request->all();
        ...
        $cie10 = DB::table('hc_cie10')->where('hc_cie10.hc_id_procedimiento', $solicitud->id_hcproc)->where('ingreso_egreso','EGRESO')->get();
        ...
        $count = Hc_Cie10::where('hc_id_procedimiento', $solicitud->id_hcproc)->where('ingreso_egreso','EGRESO')->get()->count();
        ...
        return ['count' => $count, 'id' => $id, 'cie10' => $cie10->cie10, 'descripcion' => $descripcion, 'pre_def' => $request['pre_def'], 'in_eg' => 'EGRESO','id_hcproc' => $solicitud->id_hcproc];
    }


    public function cargar_tabla_cie_alta(Request $request)
    {

        $c     = [];
        $x     = 0;
        $cie10 = DB::table('hc_cie10')->where('hc_cie10.hc_id_procedimiento', $request['id_hcproc'])->where('ingreso_egreso','EGRESO')->get();
        //dd($cie10);
        if (!is_null($cie10)) {
            foreach ($cie10 as $value) {
