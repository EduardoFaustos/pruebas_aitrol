...
class ServiciosController extends Controller
{
    ...
    public function getBanners(Request $request)
    {
        ...
        if ($global == '1') {
            ...
            $url = array();
            $banners = Apps_Banners::where('estado', 1)->where('id_empresa', '0993075000001')->get();
        ...
        if ($global == '1') {
            ...
            if ($tipo_pago == "ORD") {
                ...
                $input_ex = [
                    ...
                    'id_paciente'          => $paciente['id'],
                    'fecha_tentativa'      => $json_request['fecha_tentativa'], // json _ request men
    ...
    public function getExam(Request $request)
    {
        if ($request['city'] == '0') {
            ...
            $exam  = Examen::where('examen.estado', '1')->where('examen.valor', '>', 0)->where('examen.id', '<>', '1124')->join('examen_agrupador_sabana as s', 's.id_examen', 'examen.id')->where('s.estado', 1)->select('examen.id', 'examen.valor', 'examen.nombre', DB::raw('SUBSTRING(examen.nombre, 1, 1) AS initial'))->orderBy('examen.nombre')->get();
            $juank = array();
    ...
    public function myExam(Request $request)
    {
        ...
        if ($global == '1') {
            ...
            foreach ($ordenes as $value) {
                ...
                $cabecera['urlPDF']  = 'http://ieced.siaam.ec/sis_medico/public/api/previsualizedExam/'.$value->id;
                $cabecera['id']      = $value->id;
    ...
    {
        //dd($this->puede_imprimir($id));
        $varvalid=$this->puede_imprimir($id);
        //dd($varvalid);
/*         $pct = $varvalid['certificados'] / $varvalid['cant_par'] * 100;
        if($pct<100|| $id=="achilan"){
            //dd('aqui');
            return view('iecedapps.nodata');
        } */
    ...
    public function store_ratings(Request $request)
    {
        $json_request = json_decode($request->getContent(), true);
        Apps_Ratings::create([
            'id_orden' => $json_request['id_agenda'],
            'value' => $json_request['value1'],
            'estado' => 1,
    ...
    public function verify_rating(Request $request)
    {
        if (!is_null($request->id)) {
            $rating = Apps_Ratings::where('id_orden', $request->id)->first();
            if (!is_null($rating)) {
    ...
    public function informacion(){
        $informacion= Apps_Informacion::where('id_empresa','0993075000001')->get();
        $p=array();
        foreach($informacion as $i){
            $k['ciudad']=$i['ciudad'];
            $k['whatsapp']=$i['whatsapp'];
            $k['whatsapp_numero']=$i['whatsapp_numero'];
            $k['telefono']=$i['telefono'];
            $k['email']=$i['email'];
            $k['nombre']= $i['nombre'];
            $k['ubicacion']=$i['ubicacion'];
            $k['imagen']= asset('').'../storage/app/avatars/'.$i->imagen;
            $k['direccion']=$i['direccion'];
            array_push($p,$k);
        }
        return response()->json(['informacion'=>$p]);
    }
    public function addPlan(Request $request){
        $user= User::find($request['idUser']);
        if(!is_null($user)){
            if($user->id!=$request['id']){
                $valid= Apps_Plan_Miembros::where('id_usuario',$request['id'])->where('id_referencia',$request['idUser'])->where('estado',1)->first();
                if(is_null($valid)){
                    ...
                    if(is_null($existUser)){
                        User::create([
                            'id'               => $request['id'],
                            'email'            => $request['email'],
                            'nombre1'          => strtoupper($request['names']),
                            'id_tipo_usuario'  => '2',
                            'nombre2'          => '',
                            'apellido1'        => strtoupper($request['surnames']),
                            'apellido2'        => '',
                            'estado'           => '1',
                            'imagen_url'       => 'avatar.jpg',
                    ...
                    $extension= 'png';
                    $imageName = date('YmdHis') .'CIA'. $request['id'];
                    $user = User::find($request['id']);
                    $r1 = Storage::disk('public')->put($imageName .'.'.$extension,base64_decode($request['filename']));
                    $planFamiliar= new Apps_Plan_Miembros;
                    $planFamiliar->id_usuario= $request['id'];
                    $planFamiliar->id_plan= $request['idPlan'];
                    $planFamiliar->id_referencia= $request['idUser'];
                    $planFamiliar->cedula= $request['id'];
                    $planFamiliar->telefono= $request['telefono'];
                    $planFamiliar->url_cedula=$imageName .'.'.$extension;
                    $planFamiliar->fecha_nacimiento= $request['date'];
                    $planFamiliar->direccion= $request['direccion'];
                    $planFamiliar->correo= $request['email'];
                    $planFamiliar->pass=$request['pass'];
                    $planFamiliar->nombre= $request['names'];
                    $planFamiliar->apellido= $request['surnames'];
                    $planFamiliar->save();
                    return response()->json(['status'=>1]);
                }else{
                    return response()->json(['status'=>0,'msj'=>'No puede registrarse el mismo usuario.']);
                }
            ...
            return response()->json(['status'=>0,'msj'=>'No existe usuario.','request'=>$request->all()]);
        }
    }
    public function loadPlan(Request $request){
        $familiars= Apps_Plan_Miembros::where('id_referencia',$request['id'])->where('estado',1)->get();
        return response()->json(['data'=>$familiars,'status'=>1]);
    }
    public function removePlan(Request $request){
        $plan= Apps_Plan_Miembros::find($request['id']);
        $plan->estado=0;
        $plan->save();
