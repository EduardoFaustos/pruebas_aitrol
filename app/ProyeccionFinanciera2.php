...
class ProyeccionFinanciera2
{
    ...
    public static function detalle_principal($desde, $hasta, $tipo, $cuentas_detalle = "")
    {
        ...
        foreach ($fechagrupo as $key => $anio) {
            ...
            $toting = EstadoResultado::detalle_total_cuenta($fech, $fech2, 'I', $afecha);
            $totcos = EstadoResultado::detalle_total_cuenta($fech, $fech2, 'C', $afecha);
            $totgas = EstadoResultado::detalle_total_cuenta($fech, $fech2, 'G', $afecha);
            $totpyg                = round(($toting - ($totcos + $totgas)) * 0.15, 2);
            $datai["trabajadores"] = ($totpyg);
            $utilidad              = $toting - ($totcos + $totgas);
            $datai["utilidad"]     = ($utilidad);

            // dd($trabajopart);
            if ($datacount > 0) {
                //dd($datac[$anio-1][0]["costos"]);
                //$vventas=$ventasant;
                $vtrabajo    = $trabajosant;
                $vutilidades = $utilidadsant;
                //trabajosant=0;
            ...
            //$aingresos+=$ventas;
            $utilidadsant += $utilidad;
            $trabajosant += $totpyg;

            //$datai["vingresos"]= ($vventas != 0) ?  number_format((($ventas/$vventas)-1)*100,0): 0.00;
            $datai["variaciony"] = ($trabajosant != 0) ? number_format((($totpyg / $trabajosant) - 1) * 100, 0) : 0.00;
            $datai["variacionx"] = ($utilidadsant != 0) ? number_format((($utilidad / $utilidadsant) - 1) * 100, 0) : 0.00;
            $datai["renta"]      = number_format(($utilidad - $totpyg), 2);
            //$datai["variaciony"]= ($vgastos != 0) ?  number_format((($gastos/$vgastos)-1)*100,0): 0.00;

            $datai["xy"] = (($utilidad * $totpyg));
            //$datai["x2"]=(($ventas*$ventas));
            $datai["y2"] = (($totpyg * $totpyg));
            //$datai["xy"]=(($utilidad_ejer*$gastos));
            $datai["x2"] = (($utilidad * $utilidad));
            //$datai["y2"]=(($gastos*$gastos));

            //$datai["aingresos"]=($ventas);
            $datai["atrabajo"]  = ($totpyg);
            $datai["autilidad"] = ($utilidad);
            //$datai["agastos"]=($gastos);

            //$datai["avingresos"]=($vventas != 0) ? (($ventas/$vventas)-1)*100: 0.00;
            $datai["vtrabajo"]    = ($trabajosant != 0) ? (($totpyg / $trabajosant) - 1) * 100 : 0.00;
            $datai["vutilidades"] = ($utilidadsant != 0) ? (($utilidad / $utilidadsant) - 1) * 100 : 0.00;
