...
<section class="content">
        ...
        <form class="form-vertical" id="form">
            ...
            <div class="col-md-12 table-responsive">
                ...
                <div class="panel panel-default">
                    <div style="margin-top: 5px;">
                        <span style="font-family: Helvetica; font-weight: bold; font-style: italic; padding-top:10px; margin-left: 4px;">{{$paciente->nombre1}} {{$paciente->apellido1}}</span>
                    </div>
                    <input type="hidden" name="paciente[]" value="{{$key}}">
                    ...
                    <div class="panel-body" style="padding:0;">
                        ...
                        <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                            <thead>
                                <tr class='well-dark'>
                                    ...
                                    <th width="10%" tabindex="0">
                                        <button onclick="nuevo({{$contador}},'{{$key}}')" type="button" class="btn btn-success btn-gray">
                            ...
                            <tbody id="agregar_cuentas{{$contador}}">
                                ...
                                <tr class="well">
                                    @php
                                    $nombre_procedimients= DB::table('hc_procedimiento_final as final')->where('final.id_hc_procedimientos',$v->id_hc_procedimientos)->join('procedimiento as p','p.id','final.id_procedimiento')->select('p.nombre')->get();
                                    $nombreconcat="";
                                    //dd($nombre_procedimients);
                                    foreach($nombre_procedimients as $x){
                                    $nombreconcat.=$x->nombre."+";
                                    }
                                    @endphp
                                    <td style="max-width:100px;">
                                        <input type="hidden" name="codigo[]" class="codigo_producto" />
                                        <input type="hidden" name="paciente[]" value="{{$v->id_paciente}}">
                                        <input type="hidden" name="nombre_procedimiento[]" value="">
                                        <input type="hidden" name="fecha_procedimiento[]" value="{{substr($v->fecha_procedimiento,0,10)}}">
                                        <input type="hidden" name="id_agenda[]" value="{{$v->id_agenda}}">
                                        <input type="hidden" name="hc_procedimiento[]" value="{{$v->id_hc_procedimientos}}">

                                        <select name="nombre[]" class="form-control select2_cuentas" style="width:100%" required onchange="verificar(this)">
                                            <option> </option>
                                            @foreach($productos as $value)
                                            <option value="{{$value->nombre}}" @if($v->codigo_producto==$value->codigo) selected="selected" @endif data-name="{{$value->nombre}}" data-codigo="{{$value->codigo}}" data-descuento="{{$value->mod_desc}}" data-precio="{{$value->mod_precio}}" data-maxdesc="{{$value->descuento}}" data-iva="{{$value->iva}}">{{$value->codigo}} | {{$value->descripcion}}</option>
                                            @endforeach

                                        </select>
                                        <textarea rows="3" name="descrip_prod[]" class="form-control px-1 desc_producto" placeholder="Detalle del producto">{{$v->nombre1}} {{$v->apellido1}} - {{$nombreconcat}} {{substr($v->fecha_procedimiento,0,10)}}</textarea>
                                        <input type="hidden" name="iva[]" class="iva" />
                                    </td>
                                    <td>
                                        <input class="form-control text-right cneto" type="text" style="width: 80%;height:20px;" onkeypress="return isNumberKey(event)" onblur="this.value=parseFloat(this.value).toFixed(0);" value="{{$v->cantidad_total}}" name="cantidad[]" required>
                                    </td>
                                    <td id="tprecio">
                                        <input type="text" class="form-control pneto" name="precio[]" onblur="this.value=parseFloat(this.value).toFixed(2);" value="{{round($v->precio_final,2)}}" style="width:40%;display:inline;height:20px;">
                                        <button type="button" disabled class="btn btn-info btn-gray btn-xs cp">
                                    ...
                                    <td>
                                        @php
                                        $tots= $v->cantidad_total*$v->precio_final;
                                        @endphp
                                        <input class="form-control px-1 text-right" type="text" style="height:20px;" onkeypress="return isNumberKey(event)" value="{{round($tots,2)}}" onblur="this.value=parseFloat(this.value).toFixed(2);" name="precioneto[]" required>
                                    </td>
                                    <td>
                ...
                @endforeach
                @else
                @foreach($pacientes as $ks)
                @php
                //dd($x);
                //dd($group_procedures);
                $paciente=Sis_medico\Paciente::find($ks);
                $contador++;
                @endphp

                    <input type="hidden" name="tipo_factura" value="3">
                    <div class="panel panel-default">
                        <div style="margin-top: 5px;">
                            <span style="font-family: Helvetica; font-weight: bold; font-style: italic; padding-top:10px; margin-left: 4px;">{{$paciente->nombre1}} {{$paciente->apellido1}}</span>
                        </div>
                        <input type="hidden" name="paciente[]" value="{{$ks}}">
                            ...
                            <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                                <thead>
                                    <tr class='well-dark'>
                                        ...
                                        <th width="5%" tabindex="0">IVA</th>
                                        <th width="10%" tabindex="0">
                                            <button onclick="nuevo({{$contador}},'{{$ks}}')" type="button" class="btn btn-success btn-gray">
                                                <i class="glyphicon glyphicon-plus" aria-hidden="true"></i>
                                            </button>
                                        </th>
                                    </tr>
                                ...
                                <tbody id="agregar_cuentas{{$contador}}">
                                    <tr class="well">
                                        ...
                                        <td>
                                            <input class="form-control text-right cneto" type="text" style="width: 80%;height:20px;" onkeypress="return isNumberKey(event)" onblur="this.value=parseFloat(this.value).toFixed(0);" name="cantidad[]" value="1" required>
                                        </td>
                                        <td id="tprecio">
                                            <input type="text" class="form-control pneto" name="precio[]" onblur="this.value=parseFloat(this.value).toFixed(2);" style="width:40%;display:inline;height:20px;" value="0">
                                            <button type="button" disabled class="btn btn-info btn-gray btn-xs cp">
                                                <i class="glyphicon glyphicon-pencil" aria-hidden="true"></i>
                                            </button>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <input class="form-control px-1 text-right" type="text" style="height:20px;" onkeypress="return isNumberKey(event)" onblur="this.value=parseFloat(this.value).toFixed(2);" name="precioneto[]" required>
                                        </td>
                                        <td>
                                            <input class="form chx" type="checkbox" style="width: 80%;height:20px;" name="valoriva[]">
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body" style="padding:0;">
                        ...
                        <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                            ...
                            @foreach($group_procedures as $ke=>$s)
                            @php
                            //dd($s);
                            @endphp
                            ...
                            //dd($group);
                            $zs= DB::table('hc_procedimiento_final as final')->where('final.id_hc_procedimientos',$s)->join('procedimiento as p','p.id','final.id_procedimiento')->select('p.nombre')->get();
                            $nombre_procedimiento="";
                            $nombre_procedimiento_final="";
                            foreach($zs as $sa){
                            $nombre_procedimiento.=" ".strtoupper($sa->nombre);
                            $nombre_procedimiento_final=strtoupper($sa->nombre);
                            }

                            //dd($nombre_procedimiento);
                            @endphp
                            <tbody id="agregar_cuentas{{$contador}}">
                                <tr class="well">
                                    ...
                                    <td style="max-width:100px;">
                                        <input type="hidden" name="codigo[]" class="codigo_producto" />
                                        <input type="hidden" name="nombre_procedimiento[]" value="{{$nombre_procedimiento_final}}">
                                        <select name="nombre[]" class="form-control select2_cuentas" style="width:100%" required onchange="verificar(this)">
                                            <option> </option>
                                            @foreach($productos as $value)

                                            <option @if($nombre_procedimiento_final==$value->nombre) selected="selected" @endif value="{{$value->nombre}}" data-name="{{$value->nombre}}" data-codigo="{{$value->codigo}}" data-descuento="{{$value->mod_desc}}" data-precio="{{$value->mod_precio}}" data-maxdesc="{{$value->descuento}}" data-iva="{{$value->iva}}">{{$value->codigo}} | {{$value->descripcion}}</option>
                                            @endforeach
            ...
            <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                ...
                <tbody id="agregar_cuentas">
                    ...
                    <input type="hidden" name="ti_factura" value="3" />
    ...
    <script type="text/javascript">
        ...
        function nuevo(id, patient) {
            ...
            rowk.innerHTML = fila;
            var eq = "<input type='hidden' name='patientf[]' value='" + patient + "'>";
        ...
        function actualizarCodigosProductos() {
            $('.codigo_producto').each(function(i, obj) {
                if ($(this).val() == "") {
                    var item = $(this).parent().find('.select2_cuentas').val();
                    var ite = $(this).parent().find('.select2_cuentas').find("option[value='CONSULTA']").data("codigo");
