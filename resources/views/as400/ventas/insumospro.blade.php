                ...
                <div class="form-group col-xs-6  col-md-2  px-1">
                    ...
                    <div class="col-md-12 px-0">
                        <select class="form-control" name="tipo_servicio" id="tipo_servicio" required>
                            ...
                            <option @if(($tipox)==3) selected="selected" @endif value="3">Todos</option>
                        </select>
            ...
            <div class="col-md-12" id="items" style=" height: 400px; overflow-y: scroll;">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        ...
                        <input type="hidden" class="patients" name="id_orden[]" value="{{$x->id}}">
                                    ...
                                    <tbody id="entrega{{$contador}}">
                                        ...
                                        <tr>
                                            <td style="max-width:100px;">
                                                ...
                                                <select class="form-control select2 productos" style="width:100%; height:20px;" required onchange="verificar(this)">
                                                    <option> </option>
                                                    @foreach($productos as $value)

                                                    <option @if(($value->codigo)==($z->id_ct_productos)) selected="selected" @endif value="{{$value->codigo}}" data-name="{{$value->nombre}}" data-codigo="{{$value->nombre}}" data-descuento="{{$value->mod_desc}}" data-precio="{{$value->mod_precio}}" data-maxdesc="{{$value->descuento}}" data-iva="{{$value->iva}}">{{$value->codigo}} | {{$value->descripcion}}</option>
                                                    @endforeach
                                            ...
                                            <td>
                                                <input class="form-control text-right cneto" type="text" style="width: 80%;height:20px;" onkeypress="return isNumberKey(event)" onblur="this.value=parseFloat(this.value).toFixed(0);" value="@if(!is_null($z->cantidad)) 1 @endif" required>
                                            </td>
                                            <td>
                                                @php
                                                $precio= $z->precio;
                ...
                <table id="example1" role="grid" class="table table-bordered table-hover dataTable" aria-describedby="example1_info" style="margin-top:0 !important">
                    ...
                    <tbody id="agregar_pago">
                        <tr style="display:none" id="mifila">
                            ...
                            <td>
                                <input class="form-control text-right cneto" type="text" style="width: 80%;height:20px;" onkeypress="return isNumberKey(event)" onblur="this.value=parseFloat(this.value).toFixed(0);" value="1" required>
                            </td>
                            <td>
    ...
    <script type="text/javascript">
        ...
        $(".btn_add").click(function() {
            ...
            if ($("#form").valid()) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('ventasConglomerada.modal_preview')}}",
                    headers: {
                        'X-CSRF-TOKEN': $('input[name=_token]').val()
                    },
                    data: $("#form").serialize(),
        ...
        function consultas() {
            ...
            var html = '<div class="panel panel-default">' +
                ...
                '<th width="10%" tabindex="0">Cantidad</th><th width="10%"  tabindex="0">Precio</th>' +
                '<th width="10%" tabindex="0">Cobrar Seguro</th><th width="10%"  tabindex="0">% Desc</th>' +
                '<th width="10%" tabindex="0">Descuento</th><th width="10%"  tabindex="0">Precio Neto</th>' +
                '<th width="5%" tabindex="0">IVA</th><th width="10%"  tabindex="0">' +
                '</th></tr>' +
                '</thead><tbody>' +
                e.innerHTML +
                '</tbody></table>' +
