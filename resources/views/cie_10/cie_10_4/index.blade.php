...
<section class="content">
  <div class="box">
    <div class="box-header">
      <div class="row">
        <div class="col-md-6">
          <h3 class="box-title">{{trans('cie10_4.listadoderegistrocie10_4')}}</h3>
        </div>
        <div class="form-group col-md-3">
          <a class="btn btn-primary" href="{{ route('cie_10_4.create')}}">{{trans('cie10_4.agregar')}}</a>
        </div>
      </div>
    </div>
    <!-- /.box-header -->
    <div class="box-body">
      ...
      <form method="POST" action="{{route('cie_10_4.search')}}">
        ...
        <div class="form-group col-md-6">
          <label for="examen" class="col-md-2 control-label">{{trans('cie10_4.nombrecorto')}}</label>
          <div class="col-md-10">
        ...
        <button type="submit" class="btn btn-primary" id="boton_buscar">
          <span class="glyphicon glyphicon-search" aria-hidden="true"></span> {{trans('cie10_4.buscar')}}</button>
      </form>
      <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
        <div class="row">
          <div class="table-responsive col-md-12">
            <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info" style="font-size: 12px;">
              <thead>
                <tr role="row">
                  <th>{{trans('cie10_4.cie103')}}</th>
                  <th>{{trans('cie10_4.nombrecorto')}}</th>
                  <th>{{trans('cie10_4.nombrelargo')}}</th>

                  <th>{{trans('cie10_4.accion')}}</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <td>
                    <div class="form-group col-md-3">
                      ...
                      <a href="{{ route('cie_10_4.edit', ['id' => $value->id]) }}" class="btn btn-block btn-warning btn-xs">
                        <span class="glyphicon glyphicon-edit"></span> {{trans('cie10_4.editar')}}
                      </a>
                    </div>
                    <!--div class="form-group col-md-3">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        ...
        <div class="row">
          <div class="col-sm-5">
            <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">{{trans('cie10_4.mostrando')}} 1 al {{count($cie_10_4)}} de {{$cie_10_4->total()}} {{trans('cie10_4.registros')}}</div>
          </div>
          <div class="col-sm-7">
