@extends('hc_admision.admision.base')
  ...
  <div class="box">
    <div class="box-header">
      <div class="row">
        ...
        <div class="form-group col-md-2 col-sm-2 col-xs-2">
            <a class="btn btn-primary btn-sm"  id="imprimir_etiquetas" target="_blank" href="{{ route('admision.etiqueta2', ['id' => $historia->id_agenda, 'seguro' => $historia->id_seguro, 'alergia' => $dato_alergia]) }}" ><span class="glyphicon glyphicon-print"></span> {{trans('admision.GenerarEtiqueta')}} </a>
        </div>
        @endif
        <div class="col-md-1 col-sm-1 col-xs-1" style="text-align: center;">
            <a  class="btn btn-primary btn-sm"  href="{{ route('controldoc.imprimirpdf_resumen', ['id' => $historia->id_agenda]) }}" ><span class="glyphicon glyphicon-print"></span> {{trans('admision.Resumen')}}</a>
        </div>
        <div class="col-md-2 col-sm-2 col-xs-2" style="text-align: center;">
            <a  class="btn btn-primary btn-sm"  href="{{ route('controldoc.imprimirdatos_paciente', ['id' => $historia->id_paciente]) }}" target="_blank" ><span class="glyphicon glyphicon-print"></span> {{trans('admision.DatosPaciente')}} </a>
        </div>
        @php $sala = Sis_medico\Sala::find($historia->idsala) @endphp
        @if($sala->hospital->nombre_hospital!='HOSPITAL')
        <div class="col-md-2 col-sm-2 col-xs-2">
          <a  href="{{url('controldoc/admision')}}/{{$hcid}}/{{$url_doctor}}/{{$unix}}"><button type="button" class="btn btn-primary btn-sm" ><span class="glyphicon glyphicon-chevron-right"></span>{{trans('admision.Continuar')}} </button></a>
        </div>
        @else
        <div class="col-md-2 col-sm-2 col-xs-2">
          <a  href="{{route('cuarto.agenda_hospital',['id_sala' => $historia->idsala])}}"><button type="button" class="btn btn-primary btn-sm" ><span class="glyphicon glyphicon-chevron-right"></span> {{trans('admision.Continuar')}}</button></a>
        </div>
        @endif
      </div>
    </div>
    ...
    <div class="box-body">
        ...
        <div class="table-responsive col-md-12">
          <table class="table table-bordered table-hover dataTable" >
            <tbody style="font-size: 12px;">
              <tr id="t1">
                <td  ><b>{{trans('admision.Paciente')}}</b></td>
              ...
              <tr >
                <td ><b>{{trans('admision.Seguro')}}</b></td>
                ...
                @if($historia->proc_consul=='1')
                <td ><b>{{trans('admision.Procedimientos')}}</b></td>
      ...
      <form id="modificar_fecha">
        ...
        <div class="col-md-3">
          <div class="col-md-6">
            <label>{{trans('admision.FechaImprimible')}} </label>
          </div>
          <div class="col-md-6">
      ...
      <form id="mod_seg_emp">
        <div class="col-md-6">
          <div class="col-md-3">
            <label>{{trans('admision.Seguro')}}</label>
          </div>
          <div class="col-md-3">
          ...
          <div class="col-md-3">
            <label>{{trans('admision.Empresa')}}</label>
          </div>
          <div class="col-md-3">
...
function cambio_seg_emp(){
      $.ajax({
          type: 'post',
          url:'{{route("controldoc.seguro_empresa")}}',
          headers:{'X-CSRF-TOKEN':$('input[name=_token]').val()},
          datatype: 'json',
