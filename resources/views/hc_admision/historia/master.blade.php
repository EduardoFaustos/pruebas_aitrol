                ...
                @endphp @if(!is_null($texto)) {{$texto}} @else NO INGRESADO @endif @else{{$procedimiento->nombre_general}}@endif</td>
                    ...
                    <tr>
                      ...
                      <td style="display: none;" id="{{$procedimiento->id}}" colspan="10">
                        ...
                        @if (isset($value->id))
                        <a class="btn btn-warning col-md-3 col-xs-3 btn-margin btn-gray" data-remote="{{route('venta.planilla.detalle',['id' => $value->id])}}" data-toggle="modal" data-target="#md_planilla" style="font-size: 10px;padding-left: 2px;padding-right: 20px;">Panillas</a>
                        @endif  
                      </td> 
                    </tr>  
                  @endforeach
