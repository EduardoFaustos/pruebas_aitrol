			...
			@if($protocolo->procedimiento->procedimiento_completo != "") 
				@if($protocolo->procedimiento->procedimiento_completo->grupo_procedimiento->imagen != "")
					@if($i < 4)
						...
						<tr style="padding: 0;">
						@endif
						<td style="margin: 0;padding: 0 4px 0px 0px;"><img style="width: 185px; height: 185px;" src="{{base_path().'/storage/app/procedimiento_completo/'.$protocolo->procedimiento->procedimiento_completo->grupo_procedimiento->imagen}}" hspace=12></td>
						@if($j >= 3)
						</tr>
						@endif
				...
				@if($i < 4)
					...
					@endif
						...
						<td style="margin: 0;padding: 0 4px 0px 0px;">
							<div style="overflow: hidden; width: 190px; height: 185px; @if($j == 1){{'margin-left: -0px;'}}@elseif($j == 2) @if($imagenes[0]->seleccionado_recortada == 0){{'margin-left: -15px;'}}@else{{'margin-left: -71px;'}}@endif @else @if($imagenes[1]->seleccionado_recortada == 0){{'margin-left: -15px;'}}@else{{'margin-left: -71px;'}}@endif @endif  ">
