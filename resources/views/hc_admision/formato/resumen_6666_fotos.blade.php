...
$adicionales = \Sis_medico\Hc_Procedimiento_Final::where('id_hc_procedimientos', $protocolo->procedimiento->id)->get();
...
?><?php echo e($texto); ?>@endif</b></td>
		...
		<br>
		<div style="position: relative;margin: 0; padding: 0; width: 100%;margin-left: -31px; margin-top: 37px;height: 514px; ">

			@php  $i = 0;   $j = 0;@endphp
			@foreach($imagenes as $value)
				@if($i < 6)
				<div style="height: 252px; display:inline-block; margin: 0;padding: 0; @if($i < 3) margin-right: -27px @elseif($i == 3) margin-right: -27px; margin-left: -252px; @endif ">
					@if($j == 0)
					@endif
						@if($value->seleccionado_recortada == 1)

						<div style="position:relative;overflow: hidden; width: 252px; height: 252px; margin: 0;padding: 0;">
							<img style="width: 355px; height: 248px; margin:0;" src="{{base_path().'/storage/app/hc_ima/'.$value->nombre}}" hspace=12>
						</div>
						@else
						<div style="position:relative;overflow: hidden; width: 252px; height: 252px;  margin: 0;padding: 0;">
							<img style="width: 240px; height: 252px;margin:0;" src="{{base_path().'/storage/app/hc_ima/'.$value->nombre}}" hspace=12>
						</div>
						@endif
