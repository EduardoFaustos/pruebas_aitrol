          ...
          <table id="example2" class="table table-bordered table-hover" style="font-size: 11px;">
            <thead>
              <tr  >
                ...
                @if($rolUsuario == 1 or $idusuario == '0931563241') <th> Modo</th>  @endif
                <!--th >Log</th-->
              </tr>
            </thead>
            <tbody >
            @foreach ($pentax as $value)
              @php $agprocedimientos = Sis_medico\AgendaProcedimiento::where('id_agenda',$value->id)->get();
                ...
                //Planilla
                $planillaExiste = Sis_medico\Planilla::where('id_agenda',$value->id)->where('aprobado',1)->first();
              @endphp
                ...
                <tr style="color: {{$p_color1}};">
                  ...
                  <!--td> Dr. PENTAX</td-->
                  <td >@if($value->id_doctor1!='9666666666') {{ $value->dapellido1 }} {{ $value->dnombre1 }} @endif</td>
                  ...
                  <!--td > </td-->
                @if(!is_null($planillaExiste))  @if($idusuario == '09315632411')<td><i style="color:red" class="fa fa-heart" aria-hidden="true"></i> </td>   @elseif($rolUsuario == 1 or $idusuario == '0931563241' ) <td> <i style="color:green" class="fa fa-check-circle-o" aria-hidden="true"></i></td>   @endif @endif
              ...
              <tr
                ...
                @endif>
                  ...
                  <td>
                        ...
                        @if($pantallapentax->id_doctor1!='9666666666') {{$xdoctor->apellido1}} @if($xdoctor->apellido2!='(N/A)'){{$xdoctor->apellido2}}@endif {{$xdoctor->nombre1}} @endif
                  ...
                  <td>  @if($pantallapentax->id_doctor3!="")
                    @php $doctor3=$doctores->find($pantallapentax->id_doctor3) @endphp
                        @if($doctor3->id_tipo_usuario=='3')Dr(a). @else Enf. @endif {{$doctor3->apellido1}} {{$doctor3->nombre1}} </a>
                  ...
                  <td>  @if($pantallapentax->id_doctor4!="")
                    @php $doctor4=$doctores->find($pantallapentax->id_doctor4) @endphp
                        @if($doctor4->id_tipo_usuario=='3')Dr(a). @else Enf. @endif {{$doctor4->apellido1}} {{$doctor4->nombre1}} </a>
                  ...
                  @if(!is_null($planillaExiste))  @if($idusuario == '0931563241')<td><i style="color:red" class="fa fa-heart" aria-hidden="true"></i> </td>   @elseif($rolUsuario == 1) <td> <i style="color:green" class="fa fa-check-circle-o" aria-hidden="true"></i></td>   @endif @endif
              </tr>
              @endif
            @endforeach
