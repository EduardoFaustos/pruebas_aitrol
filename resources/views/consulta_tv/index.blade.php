            ...
            <table id="example2" class="table table-bordered table-hover" style="font-size: 12px;">
                <thead>
                    <tr>
                        ...
                        <th>Medico 1</th>
                        <th class="medico2">Medico 2</th>
                        <th class="enfermero">Enfermero</th>
                        <th class="anestesiologo">Anestesiologo</th>
                        <th>Convenio</th>
                        <th>Estado</th>
                        <th>Log</th>
                    </tr>
                ...
                <tbody>
                    ...
                    $p_color1="black"; if($value->solo_robles == '1'){ $p_color1='purple'; } else {
                    if($value->supervisa_robles == '1'){ $p_color1='red'; } else {$p_color1=$value->scolor;} };
                    ...
                    <tr style="color: {{$p_color1}};">
                        ...
                        <td>@if($value->est_amb_hos == 0)AMBULATORIO @else @if($value->omni=='SI') HOSP/OMNI @else
                            HOSPITALIZADO @endif @endif</td>
                        <td>@if($value->id_doctor1!='9666666666') {{ $value->dapellido1 }} {{ $value->dnombre1 }} @endif
                        </td>
                        <!--td>DR PENTAX</td-->
                        <td>{{ $value->d2nombre1 }} {{ $value->d2apellido1 }}</td>
                        <td class="medico2">{{ $value->d3nombre1 }} {{ $value->d3apellido1 }}</td>
                        <td class="enfermero">&nbsp</td>
                        <td class="anestesiologo">&nbsp</td>
                    ...
                    $p_color2="black"; if($value->solo_robles == '1'){ $p_color2='purple'; } else {
                    if($value->supervisa_robles == '1'){ $p_color2='red'; } else {$p_color2=$ptx_seg->color;} };
                    @endphp
                    ...
                    <tr @if($pantallapentax->estado_pentax == '-1') @elseif($pantallapentax->estado_pentax == '0')
                        ...
                        @elseif($pantallapentax->estado_pentax < '3' )
                            ...
                            <td>
                                ...
                                <a style="text-decoration:underline; color:{{$p_color2}};"
                                    href="javascript:cambia_estado({{$pantallapentax->id}}, document.getElementById('estado_pentax{{$value->id}}').value)">@if($pantallapentax->id_doctor1=='9666666666')
                                    Seleccione ... @else @if(!is_null($xdoctor)) {{$xdoctor->apellido1}}
                            ...
                            <td>
                                ...
                                @if($pantallapentax->estado_pentax!='4' && $pantallapentax->estado_pentax!='5')
                                <a style="text-decoration:underline; color:{{$p_color2}};"
                                    href="javascript:cambia_estado({{$pantallapentax->id}}, document.getElementById('estado_pentax{{$value->id}}').value)">@if(!is_null($doctor2))
                                    @if($doctor2->id_tipo_usuario=='3')Dr(a). @else Enf. @endif {{$doctor2->apellido1}}
                                    {{$doctor2->nombre1}} @endif</a>
                                @else
                                @if(!is_null($doctor2))
                                @if($doctor2->id_tipo_usuario=='3')Dr(a). @else Enf. @endif {{$doctor2->apellido1}}
                                {{$doctor2->nombre1}}
                            ...
                            <td class="medico2"> @if($pantallapentax->id_doctor3!="")

                                @php $doctor3=$doctores->find($pantallapentax->id_doctor3) @endphp
                                @if($pantallapentax->estado_pentax!='4' && $pantallapentax->estado_pentax!='5')

                                <a style="text-decoration:underline; color:{{$p_color2}};"
                                    href="javascript:cambia_estado({{$pantallapentax->id}}, document.getElementById('estado_pentax{{$value->id}}').value)">@if(!is_null($doctor3))
                                    @if($doctor3->id_tipo_usuario=='3')Dr(a). @else Enf. @endif {{$doctor3->apellido1}}
                                    {{$doctor3->nombre1}} @endif</a>
                            ...
                            <td class="enfermero"> @if($pantallapentax->id_doctor4!="")

                                @php $doctor4=$doctores->find($pantallapentax->id_doctor4) @endphp
                                @if($pantallapentax->estado_pentax!='4' && $pantallapentax->estado_pentax!='5')

                                <a style="text-decoration:underline; color:{{$p_color2}};"
                                    href="javascript:cambia_estado({{$pantallapentax->id}}, document.getElementById('estado_pentax{{$value->id}}').value)">@if(!is_null($doctor4))
                                    @if($doctor4->id_tipo_usuario=='3')Dr(a). @else Enf. @endif {{$doctor4->apellido1}}
                                    {{$doctor4->nombre1}} @endif</a>
                            ...
                            <td class="anestesiologo"> @if($pantallapentax->id_anestesiologo!="")

                                @php
                                $anestesiologo=null;
                                if($pantallapentax->id_anestesiologo!=null){
                                $anestesiologo=$anestesiologos->find($pantallapentax->id_anestesiologo);
                                }
                                ...
                                <a style="text-decoration:underline; color:{{$p_color2}};"
                                    href="javascript:cambia_estado({{$pantallapentax->id}}, document.getElementById('estado_pentax{{$value->id}}').value)">@if(!is_null($anestesiologo))
                                    Dr(a). {{$anestesiologo->apellido1}} {{$anestesiologo->nombre1}} @endif</a>
                            ...
                            <td>@if($pantallapentax->estado_pentax!='4' && $pantallapentax->estado_pentax!='5')
                                <select id="estado_pentax{{$value->id}}" name="estado_pentax"
                                    ...
                                    <option @if($pantallapentax->estado_pentax=='-1') selected @endif value='-1'>PRE -
                                        ADMISIONADO</option>
                                    @endif
                                    @if($pantallapentax->estado_pentax=='-1' || $pantallapentax->estado_pentax=='0' ||
                                    $pantallapentax->estado_pentax=='1' || $pantallapentax->estado_pentax=='2')
                                    ...
                                    @endif
                                    @if($pantallapentax->estado_pentax!='0' && $pantallapentax->estado_pentax!='1' &&
                                    $pantallapentax->estado_pentax!='-1')
                                    <option @if($pantallapentax->estado_pentax=='3') selected @endif
                                        value='3'>RECUPERACION</option>
                                    @endif
                                    @if($pantallapentax->estado_pentax!='0' && $pantallapentax->estado_pentax!='1' &&
                                    $pantallapentax->estado_pentax!='2' && $pantallapentax->estado_pentax!='-1')
