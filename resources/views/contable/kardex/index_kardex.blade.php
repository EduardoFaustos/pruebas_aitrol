...
<section class="content">
    ...
    <div class="box" style=" background-color: white;">
        ...
        <div class="box-body dobra">
            <form method="POST" id="reporte_master" action="{{ route('kardex_inventario.index') }}">
                {{ csrf_field() }}
                ...
                <div class="form-group col-md-6 col-xs-6" style="padding-left: 0px;padding-right: 0px;">
                    ...
                    <div class="col-md-9">
                        <select class="select2_find_producto form-control" name="id_producto" id="id_producto" style="width: 100%">
                ...
                <div class="form-group col-md-6 col-xs-6" style="padding-left: 0px;padding-right: 0px;">
                    ...
                    <div class="col-md-9">
                        <select class=" form-control" name="id_tipo" id="id_tipo" style="width: 100%">
                            ...
                            <option @if($request->id_tipo == "impor") selected @endif value="impor">Importaciones</option>
                        </select>
                    </div>
                </div>
                    ...
                    <div class="box box-solid">
                        ...
                        <div class="box-body">
                            ...
                            <div id="imprimir">
                                ...
                                <div class="col-md-4">
                                    <h4 style="text-align: center;">INFORME DE KARDEX</h4>
                                    @if(($fecha_desde!=null))
                                ...
                                <table id="example2" class="display compact" style="font-size: 12px; width: 100%;">
                                    <thead>
                                        <tr>
                                            <th></th>
                                            <th></th>
                                            <th colspan="2" style="background-color: skyblue;">Detalle de Producto</th>
                                            <th colspan="3" style="background-color:aquamarine;">Entrada</th>
                                            <th colspan="3" style="background-color: red;">Salida</th>
                                            <th colspan="3" style="background-color:silver;">{{trans('contableM.Saldos')}}</th>
                                        </tr>
                                        <tr>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach($informe as $x)
                                        @php
                                        $tipo=Sis_medico\Inventario::tipo_producto($x->id_producto);
                                        $iniciales= Sis_medico\Inventario::iniciales($x->id_producto,$empresa->id,$fecha_hasta);
                                        $saldos_cantidad=0;
                                        if(isset($iniciales['cantidad'])){
                                            if($iniciales['cantidad']>0){
                                               $saldos_cantidad= $iniciales['cantidad'];
                                            }
                                        }
                                        $saldos_precio=0;
                                        if(isset($iniciales['precio'])){
                                            if($iniciales['precio']>0){
                                              $saldos_precio= $iniciales['precio'];
                                            }
                                        }
                                        @endphp
                                       
                                        <tr>
                                            <td class="bold">Saldo Anterior</td>
                                            <td></td>
                                            <td>@if(isset($iniciales['fecha'])){{date('Y-m-d',strtotime($iniciales['fecha']))}}@endif</td>
                                            <td>Ingreso de Inventario</td>
                                            ...
                                            <td></td>
                                            <td></td>
                                            @if(isset($iniciales['fecha']))
                                            <td>{{number_format($iniciales['cantidad'],2,'.','')}}</td>
                                            <td>{{number_format($iniciales['precio'],2,'.','')}}</td>
                                            <td>{{number_format($iniciales['total'],2,'.','')}}</td>
                                            @else 
                                            <td></td>
                                            <td></td>
                                            <td></td>
                                        ...
                                        <tr class="bold">
                                            <td>{{$tipo}}</td>
                                            <td></td>
                                            <td>@if(isset($x->movimiento)) {{$x->movimiento->tipo}} / @endif</td>
                                        ...
                                        @if($p->id_transaccion==2)
                                        @php 
                                            $total= $p->cantidad * $p->precio; 
                                            $saldos_cantidad+=$p->cantidad; 
                                            $saldos_precio+=$p->precio;
                                            $saldos_total= $saldos_cantidad * $saldos_precio;
                                        @endphp
                                        <tr class="subtitle">
                                            <td></td>
                                            <td> <label class="label label-primary">FC:   @if(isset($p->movimiento->compras))  {{$p->movimiento->compras->numero}} </label> &nbsp;  <label class="label label-warning"> P: {{$p->movimiento->compras->proveedorf->razonsocial}} </label> &nbsp;  @else No data @endif @if($p->movimiento->pedido!=null) <label class="label label-info"> #P: {{$p->movimiento->pedido}} @endif </label> </td>
                                            <td>{{$p->fecha}}</td>
                                            <td>{{$x->producto->nombre}}</td>
                                            <td>{{$p->cantidad}}</td>
                                            <td>{{$p->precio}}</td>
                                            ...
                                            <td>0.00</td>
                                            <td>0.00</td>
                                            <td>{{number_format(round($saldos_cantidad,2),2)}}</td>
                                            <td>{{number_format(round($saldos_precio,2),2)}}</td>
                                            <td>{{number_format(round($saldos_total,2),2)}}</td>
                                        </tr>
                                        @else
                                        @php
                                        $saldos_precio=$p->precio;
                                        ...
                                        $saldos_total= $saldos_precio * $saldos_cantidad;
                                        @endphp
                                        <tr class="subtitle">
                                            <td></td>
                                            <td><label class="label label-primary">FV:  @if(isset($p->movimiento->ventas)) {{$p->movimiento->ventas->nro_comprobante}}  </label> &nbsp;   <label class="label label-warning">C: {{$p->movimiento->ventas->cliente->nombre}} </label>@else No data @endif @if(isset($p->movimiento->ventas)) @if(isset($p->movimiento->ventas->paciente)) <br/> &nbsp; <label class="label label-info">PCTE:  {{$p->movimiento->ventas->paciente->nombre1}} {{$p->movimiento->ventas->paciente->apellido1}} </label>  @endif @endif </td>
                                            <td>{{$p->fecha}}</td>
                                            <td>{{$x->producto->nombre}}</td>
                                            <td>0.00</td>
                                            <td>0.00</td>
...
<script type="text/javascript">
    ...
    $('.select2_find_producto').select2({
        placeholder: "Escriba el nombre del producto",
         allowClear: true,
        ajax: {
            url: '{{route("kardexinterno.productosearch")}}',
