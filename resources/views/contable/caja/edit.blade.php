...
<section class="content">
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item"><a href="../emision">{{trans('mcaja.puntodeemision')}}</a></li>
      <li class="breadcrumb-item active" aria-current="page">{{trans('mcaja.actualizar')}}</li>
    </ol>
  </nav>
  <form class="form-vertical" role="form" method="POST" action="{{route('puntoemision.update')}}">
    ...
    <div class="box">
      <div class="box-header color_cab">
        <div class="col-md-9">
          <h5><b>{{trans('mcaja.detallepuntoemision')}}</b></h5>
        </div>
      ...
      <div class="box-body dobra">
        ...
        <div class="form-group  col-xs-6">
          ...
          <div class="col-md-7">
            <input id="codigo_punto" name="codigo_punto" type="text" class="form-control" maxlength="3" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" value="@if(!is_null($caja)){{$caja->codigo_caja}}@endif" autocomplete="off" required>
          </div>
        </div>
        ...
        <div class="form-group  col-xs-6">
          ...
          <div class="col-md-7">
            <input id="nombre_punto" name="nombre_punto" type="text" class="form-control" value="@if(!is_null($caja)){{$caja->nombre_caja}}@endif" autocomplete="off" required autofocus>
          </div>
        </div>
        ...
        <div class="form-group col-md-6">
          ...
          <div class="col-md-7">
            <select id="estado_punto" name="estado_punto" class="form-control" required>
              <option {{ $caja->estado == 1 ? 'selected' : ''}} value="1">{{trans('mcaja.activo')}}</option>
              <option {{ $caja->estado == 0 ? 'selected' : ''}} value="0">{{trans('mcaja.inactivo')}}</option>
            </select>
          </div>
        </div>
        ...
        <div class="form-group col-xs-10 text-center">
          <div class="col-md-6 col-md-offset-4">
            <button type="submit" id="btn_add" class="btn btn-default btn-gray btn_add">
              <i class="glyphicon glyphicon-floppy-disk" aria-hidden="true"></i>&nbsp;&nbsp;{{trans('mcaja.actualizar')}}
            </button>
          </div>
        </div>
      </div>
