                ...
                <div class="col-md-12">
                    <div class="header row">
                            ...
                            <select style="width: 100%;" class="form-control select2" onchange="changeProveedor(this)" name="acreedores" id="acreedores">
                                ...
                                @foreach($compras as $c)
                                    @php 
                                        ...
                                        $cruce  = Sis_medico\Ct_Cruce_Cuentas::where('id_factura', $c->id)->where('estado', 1)->first();
                                      
                                    @endphp
                                    @if(is_null($cruce))
                                    @php 
                                    $retencion= Sis_medico\Ct_Retenciones::where('id_compra',$c->id)->orderBy('id','DESC')->first();
                                    @endphp
                                    @if(!is_null($retencion))
                                        @if($retencion->estado==0 && $retencion->anulado==0)
                                        <option data-subtotal="{{$c->subtotal}}" data-proveedor="{{$c->proveedor}}" data-iva="{{$c->iva_total}}" data-total="{{$c->total_final}}" data-concepto="{{$c->observacion}}" data-nombre="{{$c->proveedorf->razonsocial}}" data-autorizacion="{{$c->autorizacion}}" value="{{$c->id}}">#: {{$c->id}} P: {{$c->proveedorf->nombrecomercial}} - {{$c->numero}} |  {{$c->observacion}}  </option>
                                        @endif
                                        @if($retencion->estado==1 && $retencion->anulado==1)
                                        <option data-subtotal="{{$c->subtotal}}" data-proveedor="{{$c->proveedor}}" data-iva="{{$c->iva_total}}" data-total="{{$c->total_final}}" data-concepto="{{$c->observacion}}" data-nombre="{{$c->proveedorf->razonsocial}}" data-autorizacion="{{$c->autorizacion}}" value="{{$c->id}}"> #: {{$c->id}} P: {{$c->proveedorf->nombrecomercial}} - {{$c->numero}} |  {{$c->observacion}}  </option>
                                        @endif
                                    @else 
                                    <option data-subtotal="{{$c->subtotal}}" data-proveedor="{{$c->proveedor}}" data-iva="{{$c->iva_total}}" data-total="{{$c->total_final}}" data-concepto="{{$c->observacion}}" data-nombre="{{$c->proveedorf->razonsocial}}" data-autorizacion="{{$c->autorizacion}}" value="{{$c->id}}">#: {{$c->id}} P: {{$c->proveedorf->nombrecomercial}} - {{$c->numero}} |  {{$c->observacion}}  </option>
                        ...
                        <div class="col-md-1 px-1">
                            ...
                            <label class="switch">
                                <input class="electros" id="toggleswitch2" type="checkbox" disabled>
                                <span class="slider round"></span>
