...
<section class="content">
  ...
  <div class="box dobra" style=" background-color: white;">
    ...
    <!-- /.box-header -->
    <div class="box-body ">
      <form method="POST" id="reporte_master" action="{{ route('contable.anticipo_proveedores') }}">
        {{ csrf_field() }}
    ...
    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="col-md-4">
            <h4 style="text-align: center;">{{trans('contableM.informeanticipoproveedor')}}</h4>
            ...
            <table id="example2" class="display compact" role="grid" aria-describedby="example2_info" style="font-size: 12px; text-align:center; width:100%;">
              <thead>
                <tr >
                  ...
                  <th  style="text-align:center;" tabindex="0" aria-controls="example2" colspan="1"># {{trans('contableM.cheque')}}</th>
                  ...
                  <th  style="text-align:center;" tabindex="0" aria-controls="example2" colspan="1">{{trans('contableM.banco')}}</th>
                  <th  style="text-align:center;" tabindex="0" aria-controls="example2" colspan="1">{{trans('contableM.beneficiario')}}</th>
                  <th  style="text-align:center;" tabindex="0" aria-controls="example2" colspan="1">{{trans('contableM.observaciones')}}</th>
              ...
              <tbody>
                ...
                @foreach($informe as $x)
                  @php $z= Sis_medico\Ct_Asientos_Detalle::where('id_asiento_cabecera',$x->id_asiento_cabecera)->where('id_plan_cuenta','1.01.04.03')->first(); @endphp
                    <tr>
                      
                      <td>{{$x->id}}</td>
                      <td>{{date('d/m/Y',strtotime($x->fecha_comprobante))}}</td>
                      <td>{{$x->no_cheque}}</td>
                      
                      <td style="text-align: left;">{{$x->proveedor->id}}</td>
                      <td style="text-align: left;">{{$x->proveedor->razonsocial}}</td>
                      <td style="text-align: left;">@if(isset($x->bancoa)) {{$x->bancoa->nombre}} @endif</td>
                      <td>{{$x->beneficiario}}</td>
                      <td style="text-align: left;">{{$x->descripcion}}</td>
                      
                      <td>@if($x->valor_pago>0){{$x->valor_pago}} @else {{$z->debe}} @endif</td>
                    </tr>
                    @php if($x->valor_pago>0){ $total+= $x->valor_pago;}else{ $total+=$z->debe; }  @endphp
                @endforeach
              </tbody>
              <tfoot>
                  <tr>
...
<script type="text/javascript">
  ...
  $('#example2').DataTable({
        ...
        },
        {
          extend: 'excelHtml5',
          footer: true,
          title: 'INFORME ANTICIPO PROVEEDORES -'+'{{date("d/m/Y")}}',
        },
        {
          extend: 'csvHtml5',
          footer: true
