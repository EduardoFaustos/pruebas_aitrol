              ...
              <table id="tabla_consultas" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example1_info" style="font-size: 12px;">
                ...
                @foreach($orden_venta_seguro as $value)
                  ...
                  <tr role="row" >
                    ...
                    <td style="background-color: #ffe6e6;color: red;font-size: 15px;">$ {{number_format(round($value->total1 + $value->total2,2),2) }}</td>
                ...
                <tr>
                  <td><b>{{$cont}}</b></td>
                  <td><b>CORTESIA</b></td>
                  <td>@if(is_null($cortesia_t)) 0 @else {{$cortesia_t->cantidad}} @endif</td>
                  <td style="background-color: #ffe6e6;color: red;font-size: 15px;">$ 0.00</td>
                </tr>
                @php
                  $cont++;
                  if(!is_null($cortesia_t)){
                    $total_cantidad = $total_cantidad + $cortesia_t->cantidad;
                  }
                ...
                @endphp
                  <tr role="row" >
                    ...
                    <td style="background-color: #ffe6e6;color: red;font-size: 15px;"><b>$ {{number_format(round($total_valor,2),2) }}</b></td>
                  </tr>
                </tbody>
              </table>
            </div>
              ...
              <table id="tabla_consultas2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example1_info" style="font-size: 12px;">
                ...
                @foreach($orden_venta_tipo_total as $value)
                  ...
                  <tr role="row" >
                    <td><b>{{$cont}}</b></td>
                    <td><b>{{$value->id}}</b></td>
                    <td><b>{{$value->id_paciente}}</b></td>
                    <td><b>{{$value->orden}}</b></td>
                  </tr>

                  @php $total_cantidad = $total_cantidad + $value->cantidad; $total_valor = $total_valor + $value->total; $cont++;@endphp
                  @endif
                  @if($value->orden==null)
                  <tr role="row" >
                    <td><b>{{$cont}}</b></td>
