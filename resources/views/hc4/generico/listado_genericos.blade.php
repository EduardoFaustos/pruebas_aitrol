            ...
            <div class="row">
              ...
              <div class="col-4" style="padding-right: 5px">
                <div class="input-group">
                   <input value="@if($nombre!=''){{$nombre}}@endif" type="text" class="form-control input-sm" name="nombre" id="nombre" placeholder="generico" style="text-transform:uppercase;" onkeypress="enviar_enter(event);">
                </div>
              </div>
              <div class="col-4" style="padding-right: 5px">
                <a class="btn btn-info" style="color:white; background-color: #004AC1; border-radius: 5px; border: 2px solid white;" onclick="buscar_generico()"> <i class="fa fa-search" aria-hidden="true">
                </i> 
                &nbsp;&nbsp;&nbsp;BUSCAR&nbsp;&nbsp;&nbsp;
                </a>
              </div>
          ...
          <div class="row">
            <div class="col-6"style="padding-right: 5px" >
              <a class="btn btn-danger" onclick="crear_generico()" style="color:white; background-color:#004AC1 ;  border-radius: 5px; border: 2px solid white; width: 100%; height: 100%">
                <span class="glyphicon glyphicon-plus">&nbsp;Crear Generico</span>
              </a>
            </div>
            <div class="col-6"style="padding-right: 5px" >
            </div>
        ...
        <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap ">
          <div class="table-responsive col-md-12 col-xs-12">
            <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info" style="font-size: 14px;">
              ...
              <tbody>
                @foreach($genericos as $xgenerico)
                <tr>
                  <td class="color">{{$xgenerico->nombre}}</td>
                  <td class="color">{{$xgenerico->descripcion}}</td>
                  <td class="color">
                    <a class="btn btn-info boton-2" style="color: white; width: 100%; height: 100%" onclick="editar_generico({{$xgenerico->id}});">
            ...
            <label class="color" style="padding-left: 15px;font-size: 16px">Total de Registros: {{$genericos->count()}}</label> 
          </div>
        </div>
...
<script type="text/javascript">
  ...
  //Funcion que permite Crear el Generico
  function crear_generico(){
    $.ajax({
      type: "GET",
      url: "{{route('crear.generico_hc4')}}", 
      data: "",
      datatype: "html",
  ...
  //Funcion que permite Editar la medicina generico
  function editar_generico(id){
      $.ajax({
        type: "GET",
        url: "{{route('edit.generico')}}/"+id, 
        data: "",
        datatype: "html",
        success: function(datahtml){
          $("#area_trabajo").html(datahtml);
  ...
  //Funcion que permite buscar el Generico
  function buscar_generico(){
         $.ajax({
            type:'post',
            url:"{{route('generico.buscar_hc4')}}",
