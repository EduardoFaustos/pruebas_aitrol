...
<form id="form_buscador">
    ...
    <div class="form-group col-1" style="margin-bottom: 0;">
      <button class="btn btn-success" onclick="" type="button"><span class="glyphicon glyphicon-search"></span></button>
		...
		<div class="form-group col-3" style="margin-bottom: 0;">
			<input id="seleccionados" name="seleccionados" type="checkbox" class="flat-red" onchange="quitar_buscador();cargar_buscador();" value="1" @if(count($detalles_ch) > 0) checked @endif><label style="color: red;font-size: 14px;"> Ver Seleccionados</label>
		</div>
    ...
    <div class="form-group col-4" style="margin-bottom: 0;">
      <button type="button" class="btn btn-warning btn-sm" onclick="deseleccionar();"><span style="color: white;"> Deseleccionar</span></button>
    </div>  
...
<script type="text/javascript">
  ...
  function cargar_buscador(){
  	$.ajax({
        type: 'post',
        url:"{{route('hc4_orden_lab.buscar',['id_orden' => $orden->id])}}",
