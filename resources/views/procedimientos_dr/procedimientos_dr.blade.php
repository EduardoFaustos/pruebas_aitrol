@extends('procedimientos_dr.base')
      ...
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="form-group col-md-6 {{ $errors->has('fecha') ? ' has-error' : '' }} ">
            <label class="col-md-3 control-label">{{trans('procedimientodr.BuscarFecha')}}</label>
            <div class="col-md-6">
          ...
          <div class="col-md-2">
            <a href="javascript:carga_reporte()" class="btn btn-primary">{{trans('procedimientodr.ReporteProcedimientos')}}</a>
          </div>
          <div class="col-md-2">
            <a href="{{ url('pentaxtv') }}" class="btn btn-primary">{{trans('procedimientodr.PentaxSalaEspera')}}</a>
          </div>
          <div class="col-md-2">
            <a href="{{ url('procedimientostv_dr') }}" class="btn btn-primary">{{trans('procedimientodr.ProcedimientosTV')}}</a>
          </div>
        </div>
      </div>
    </div>
...
<script type="text/javascript">
  ...
  var findex_pentax = function() {
    ...
    $.ajax({
      type: 'get',
      url: '{{ route('procedimientos_dr.index_dr')}}/' + unix,
  ...
  function cambia_estado(id, value) {
    ...
    document.getElementById('cambios_pentax').href = "{{ route('procedimientos_dr.edit_dr')}}/" + id + "/" + value + "/" + unix;
