...
<div class="container">
    <div class="row">
        <div class="box box-primary col-xs-24">
            <div class="box-header"><h3 class="box-title">{{trans('ecortesia.EditarProcedimiento')}}</h3></div>
            <form class="form-vertical" role="form" method="POST" action="{{ route('procedimiento.update', ['id' => $procedimiento->id]) }}">
                <div class="box-body col-xs-24">
                    ...
                    <div class="form-group col-xs-12{{ $errors->has('observacion') ? ' has-error' : '' }}">
                        <label for="observacion" class="col-md-2 control-label">{{trans('econsultam.Observación')}}</label>
                        <div class="col-md-7">
                    ...
                    <div class="form-group col-xs-12{{ $errors->has('estado') ? ' has-error' : '' }}">
                        <label for="estado" class="col-md-2 control-label">{{trans('econsultam.Estado')}}</label>
                        <div class="col-md-7">
                            <select id="estado" name="estado" class="form-control">
                                <option {{$procedimiento->estado == 0 ? 'selected' : ''}} value="0">{{trans('ecamilla.INACTIVO')}}</option>
                                <option {{$procedimiento->estado == 1 ? 'selected' : ''}} value="1">{{trans('ecamilla.ACTIVO')}}</option>            
                            </select>  
