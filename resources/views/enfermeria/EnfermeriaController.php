                            ...
                            if ($secuencia != 0) {
                                ...
                                if (is_null($la_planilla)) {
                                    ...
                                    /*$cab_mov_inv                        = new InvCabMovimientos;
                                        ...
                                        $cab_mov_inv->id_transaccion_bodega = $transaccion->id;
                                        $cab_mov_inv->id_bodega_origen      = $id_bodega;
                                        $cab_mov_inv->id_bodega_destino     = $id_bodega_compra;
                                        $cab_mov_inv->numero_documento      = str_pad($secuencia, 9, "0", STR_PAD_LEFT); 
                                        $cab_mov_inv->observacion           = 'Paciente: '.$paciente->apellido1.' '.$paciente->apellido2.' '.$paciente->nombre1.' '.$paciente->nombre2;
                                        $cab_mov_inv->fecha                 = date('Y-m-d'); 
                                    ...
                                    if (!is_null($inv_serie)) {
                                        ...
                                        if (!is_null($pedido)) {
                                            ...
                                            if (!is_null($detalle_movimiento)) {
                                                ...
                                                /*$det_mov_inv                            = new InvDetMovimientos;
                                                    $det_mov_inv->id_inv_cab_movimientos    = $cab_mov_inv->id;
                                                    $det_mov_inv->id_producto               = $id_producto;
                                                    $det_mov_inv->id_inv_inventario         = $inventario->id;
                                                    $det_mov_inv->cantidad                  = $item_cant;
                                                    $det_mov_inv->cant_uso                  = $item_cant* $cant_uso;
                                                    $det_mov_inv->serie                     = $inv_serie->serie; //inv inventario
                                                    $det_mov_inv->lote                      = $inv_serie->lote;  
                                                    $det_mov_inv->fecha_vence               = $inv_serie->fecha_vence;
                                                    $det_mov_inv->valor_unitario            = $precio;
                                                    $det_mov_inv->subtotal                  = $item_cant * $precio;
                                                    $det_mov_inv->descuento                 = 0;
                                                    $det_mov_inv->iva                       = $iva;
                                                ...
                                                /*if ($inv_serie->inventario->producto->usos == 0) {
                                                        if ($inv_serie->fecha_vence >= date('Y-m-d')) {
                                                            # GENERO EL DESCARGO 
                                                            
                                                            $id_movimiento_paciente = $transitocontroller->_serie_enfermero($request); 
                                                            $transitocontroller->documentoTrasladoCompra($inv_serie,$id_agenda,$id_hc_procedimientos,1,1,$id_movimiento_paciente);
                                                            $id = $transitocontroller->documentoEgreso($inv_serie,$id_agenda,$id_hc_procedimientos,1,1,$id_movimiento_paciente);
                                                            return $id;
                                                        } else {
                                                            return "caducado";
                                                        }
                                                ...
                                                // SI NO ENCUENTRA DATOS POR NUMERO DE SERIE VERIFICA POR CODIGO DEL PRODUCTO  Y BODEGA PARA OBTENER EL INVENTARIO
                                                /*$producto   = Producto::where('codigo',$serie)->first();
                                                    $id_bodega  = env('BODEGA_EGR_PACI1',13);
                                                    if (isset($producto->id)) {
                                                        #inventario
                                                        $inventario = InvInventario::getInventario($producto->id,$id_bodega);
                                                        if(isset($inventario->id)) {
