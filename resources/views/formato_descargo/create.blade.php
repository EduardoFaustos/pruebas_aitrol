...
<section class="content">
    <div class="box " style="background-color: white;">
        ...
        <div class="box-body" style="background-color: #ffffff;">
            <form class="form-vertical" id="formi" role="form" method="POST" action="{{route('formatosProductos.store')}}">
                {{ csrf_field() }}
                <div class="box-body col-xs-24">
                    ...
                    <div class="form-group col-xs-6{{ $errors->has('descripcion') ? ' has-error' : '' }}">
                        ...
                        <div class="col-md-7">
                          <textarea class="form-control"  onkeyup="javascript:this.value=this.value.toUpperCase();" name="descripcion" id="descripcion" cols="10" rows="5"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-group col-xs-6{{ $errors->has('estado') ? ' has-error' : '' }}">
                        ...
                        <div class="col-md-7">
                             <select id="estado" name="estado" class="form-control" required>
                                <option  value="1">{{trans('ecamilla.ACTIVO')}}</option>
                                <option  value="0">{{trans('ecamilla.INACTIVO')}}</option>
                            </select>
                            ...
                            <table class="table table-striped" id="example3" role="grid" aria-describedby="example2_info" style="width: 100%;">
                                ...
                                <tbody id="det_recibido">
                                    <tr style="margin-top:10px;">
                                        <td><select class="form-control select2_cuentas" name="codigo[]" style="width: 98%;" required> <option value="">{{trans('econtrolsintomas.Seleccione')}} ...</option> @foreach($productos as $x) <option value="{{$x->id}}">{{$x->codigo}} {{$x->descripcion}}</option>  @endforeach </select></td>
                    ...
                    <div class="col-md-12" style="text-align: center;">
                        <button type="button" onclick="sumit()" class="btn btn-success btn-gray"> <i class="fa fa-save"></i> {{trans('econtrolsintomas.Guardar')}} </button>
                    </div>
                </div>
            </form>
        </div>
