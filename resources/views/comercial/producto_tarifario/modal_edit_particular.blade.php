    ...
    <form method="post" id="form_nivel" action="">
        ...
        <div class="row">
            <div id="div_precio" style="margin-bottom: 0px;" class="form-group col-md-6">
                ...
                <div class="col-md-8">
                    <input type="number" class="form-control input-sm validar" name="precio" required value="{{$producto->valor_total_paq}}">
                </div>
            </div>
        </div>
...
<div class="modal-footer">
    <button type="button" class="btn btn-primary pull-right" id="btn_guardar" onclick="guardar(event)"><span class="glyphicon glyphicon-floppy-disk"> {{trans('prod_tar.actualizar')}}</span></button>
</div>
...
<script type="text/javascript">
    ...
    function guardar(e) {
        ...
        if (!validar_campos()) {
            $.ajax({
                type: 'post',
                url: "{{ route('prodtarifario.update_particular') }}",
                headers: {
