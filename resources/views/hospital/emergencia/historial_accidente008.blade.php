...
<div class="content">
        ...
        <div class="row">
            <div class="col-md-10 col-sm-10">
                ...
                {{trans('transformularios.Formulario008')}}
                    <small>{{trans('transformularios.AccidenteViolencia')}}</small>
                </h3>
            </div>
            <div class="col-2">
	...
	<div class="row">
		...
		<!-- 4.- Enfermedad Actual y Revisión de Sistemas -->
		<div class="col-md-12">
			<div class="box box-primary">
				<div class="box-header with-border">
					<h3 class="box-title">{{trans('transformularios.HistorialAccidente')}}</h3>
					...
					<div class="box-body">
						<div class="row">
							<div class="col-md-12">
                                ...
                                @endforeach
                                <table id="accidente" class="table table-bordered table-hover table-sm">
                                    <thead>
                                        <tr>
                                            <th>{{trans('transformularios.FechayHora')}}</th>
                                            <th>{{trans('transformularios.Cedula')}}</th>
                                            <th>{{trans('transformularios.LugardelEvento')}}</th>
                                            <th>{{trans('transformularios.DireccióndelEvento')}}</th>
                                            <th>{{trans('transformularios.CustodiaPolicial')}}</th>
                                            <th>{{trans('transformularios.AlientoEtílico')}}</th>
                                            <th>{{trans('transformularios.ValorAlcocheck')}}</th>
                                    ...
                                    @foreach($dato_paciente as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->custodia_policial }}</td>
                                            <td>{{ $item->aliento_etilico }}</td>
                                            <td>{{ $item->valor_alcocheck }}</td>
