...
<body>
	...
	<div class="containerw3layouts-agileits" style="background-color: #F8F9FA;z-index: 3;width: 95%; position: relative;">
		<div class="w3layoutscontactagileits">
			<img class="centrado" style="position: absolute; top:5px; right: 19px;z-index: 2;" src="{{asset('/imagenes/endoscopynet_gastroquito.png')}}">
			<h1 style="margin: 30px 0 0 0 !important; color: black !important;z-index: 3;"></h1>
			<br>
			<br>
			<br>

			<h3 style="color: black !important;z-index: 3;height: 100% !important;">{{$encuesta->descripcion}}</h3>
			...
			<div id="wrapper">
				<div class="table-responsive col-md-12">
					<form class="form-vertical" method="get" id="enviar2">
						...
						<h3 style="color: black !important;z-index: 3;"> {{trans('encuestas.en')}} <b>{{trans('encuestas.endoscopynet')}}</b> {{trans('encuestas.nosinteresatuopinion')}}</h3>
						...
						<table>
							<tbody>
								<tr>
									<td width="25%" style="vertical-align: middle; border-color: black; font-size: 13px;">{{$dato->nombre}}</td>

									@foreach($tiempos as $value)

									<td width="14%" style="text-align: center; border-color: black;vertical-align: middle;font-size: 13px;">
										{{$value}}min <input type="radio" class="check2" name="{{$dato->id}}" value="{{$value}}">
									</td>
									@endforeach

									<td width="14%" style="text-align: center; border-color: black;vertical-align: middle; font-size: 13px;">{{trans('encuestas.otros')}}<input type="radio" class="check2" name="{{$dato->id}}" value="otros"><input type="number" class="check2" name="val2{{$dato->id}}"></td>
						...
						<table id="example2">
							<thead>
								<tr style="font-size: 13px;text-align: center;">
									<th width="30%" style=" font-size: 16px; vertical-align: middle;border-color:black;background-color: #2CB9D8;">{{trans('encuestas.criteriosdeevaluacion')}}</th>
									<th width="5%" style="padding: 0 14px;border-color:black;background-color: #2CB9D8;">{{trans('encuestas.excelente')}}</th>
									<th width="5%" style="padding: 0 14px;border-color:black;background-color: #2CB9D8;">{{trans('encuestas.muybueno')}}</th>
									<th width="5%" style="padding: 0 14px;border-color:black;background-color: #2CB9D8;">{{trans('encuestas.bueno')}}<br></th>
									<th width="5%" style="padding: 0 14px;border-color:black;background-color: #2CB9D8;">{{trans('encuestas.nibuenonimalo')}}<br></th>
									<th width="5%" style="padding: 0 14px;border-color:black;background-color: #2CB9D8;">{{trans('encuestas.malo')}} <br></th>
									<th width="5%" style="padding: 0 14px;border-color:black;background-color: #2CB9D8;">{{trans('encuestas.muymalo')}}</th>
								</tr>
							</thead>
							<tbody>
								@foreach($value as $dato)
								<tr role="row" class="odd">
									<td style="border-color:black;font-size: 13px; vertical-align: middle;">{{$dato->nombre}}</td>
									<td>
									...
									<td style="text-align: center;vertical-align: middle;border-color:black;">
										<label for="neutral{{$dato->id}}">
											<input type="radio" class="neutral" name="{{$dato->id}}" id="neutral{{$dato->id}}" value="3.5" />
											<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="100%" height="100%" viewBox="0 0 24 24">
									...
									<td style="text-align: center;vertical-align: middle;border-color:black;">
										<label for="neutral-2{{$dato->id}}">
											<input type="radio" class="neutral-2" name="{{$dato->id}}" id="neutral-2{{$dato->id}}" value="3" />
											<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="100%" height="100%" viewBox="0 0 24 24">
												<path d="M8.5,11A1.5,1.5 0 0,1 7,9.5A1.5,1.5 0 0,1 8.5,8A1.5,1.5 0 0,1 10,9.5A1.5,1.5 0 0,1 8.5,11M15.5,11A1.5,1.5 0 0,1 14,9.5A1.5,1.5 0 0,1 15.5,8A1.5,1.5 0 0,1 17,9.5A1.5,1.5 0 0,1 15.5,11M12,20A8,8 0 0,0 20,12A8,8 0 0,0 12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22C6.47,22 2,17.5 2,12A10,10 0 0,1 12,2M9,14H15A1,1 0 0,1 16,15A1,1 0 0,1 15,16H9A1,1 0 0,1 8,15A1,1 0 0,1 9,14Z" />
											</svg>
										</label>
									</td>
									<td style="text-align: center;vertical-align: middle;border-color:black;"><label for="sad{{$dato->id}}">
											<input type="radio" class="sad" name="{{$dato->id}}" id="sad{{$dato->id}}" value="2.5" />
											<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="100%" height="100%" viewBox="0 0 24 24">
				...
				<div class="wthreesubmitaits" style="margin-bottom: 50px;">
					<button class="boton btn btn-primary btn-lg" style="width: 12%;z-index: 9999;position: static; padding: 10px;color: white;border-radius: 20px;" id="boton_enviar" onclick="enviar_encuesta()">{{trans('encuestas.enviar')}}</button>
	...
	<div class="w3lsfooteragileits" style="position: relative;">
		<p style="color: black;z-index: 2; position: relative;"> &nbsp; </p>
		<!--img class="centrado" style="position: absolute; bottom: -150px; width: 100%;z-index: 99;" src="{{asset('/imagenes/footer.png')}}"-->
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data == "1") {
						...
						swal({
							title: `{{trans('encuestas.ingresecedula')}}`,
							icon: "warning",
							type: 'error',
							buttons: true,
						});
					...
					if (data == "3") {
						$('#boton_enviar').removeAttr("disabled");
						swal({
							title: `{{trans('encuestas.completeformulario')}}`,
							icon: "warning",
							type: 'error',
							buttons: true,
						});
					...
					if (data == "4") {
						$('#boton_enviar').removeAttr("disabled");
						swal({
							title: `{{trans('encuestas.noregistra')}}`,
							icon: "warning",
							type: 'error',
							buttons: true,
						});
					...
					if (data == "5") {
						$('#boton_enviar').removeAttr("disabled");
						swal({
							title: `{{trans('encuestas.pacientenoregis')}}`,
					...
					if (data == "ok") {
						swal({
								...
								title: `{{trans('encuestas.graciasmensaje')}}`,
								icon: "success",
								type: 'success',
								buttons: true,
							})
							.then((value) => {
								...
								location.href = "{{route('rrhh.encuesta', ['id'=>$encuesta->id])}}";
							});
					};
				},
				error: function(data) {
