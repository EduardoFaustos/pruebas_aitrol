...
<!-- Ventana modal de pendiente de pago -->
<div class="modal fade fullscreen-modal" id="pendiente_pago" data-keyboard="false" tabindex=null role="dialog" aria-labelledby="myModalDoctor" aria-hidden="true">
...
<div class="modal fade" id="log_muestra" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
...
<section class="content">
  <div class="box">
    <div class="box-header">
      <div class="row">
        <div class="col-md-4">
          <h3 class="box-title">{{trans('laboratorio.ordenes_laboratorio')}}</h3>
        </div>
        <div class="col-md-2">
          <a data-toggle="modal" data-target="#pendiente_pago" class="btn btn-success" href="{{ route('ordenes.modal_pendiente_pago')}}">
            <i class="fa fa-file-text" aria-hidden="true"></i> {{trans('laboratorio.pendiente_pago')}}
          </a>
        </div>
        <div class="col-md-2">
          <a class="btn btn-warning" href="{{ route('orden.create')}}"><span class="ionicons ion-ios-flask"></span> {{trans('laboratorio.orden_publica')}}</a>
        </div>
        <div class="col-md-2">  
            
          <a class="btn btn-primary" href="{{ route('orden_particular.crear_particular')}}"><span class="ionicons ion-ios-flask"></span> {{trans('laboratorio.orden_privada')}}</a>
        </div> 
        <div class="col-md-2">
          <a id="pago_on" data-toggle="modal" data-target="#pago_online" @if($gestiones->count()>0) class="btn btn-danger" @else class="btn btn-success" @endif  href="{{ route('orden.pagoenlinea_gestionar')}}">
            <span class="glyphicon glyphicon-credit-card"></span> {{trans('laboratorio.pagos_enlinea')}}
          </a>
        </div>
      </div>
    </div>
    ...
    <div class="box-body">
      <form method="POST" action="{{route('orden.search')}}">
        ...
        <div class="form-row">
          <div class="form-group col-md-2 col-xs-6">
            <label for="fecha" class="col-md-12 control-label">{{trans('laboratorio.desde')}}</label>
            <div class="col-md-12">
          ...
          <div class="form-group col-md-2 col-xs-6">
            <label for="fecha_hasta" class="col-md-12 control-label">{{trans('laboratorio.hasta')}}</label>
            <div class="col-md-12">
              <div class="input-group date">
                <div class="input-group-addon">
                  <i class="fa fa-calendar"></i>
          ...
          <div class="form-group col-md-2 col-xs-6">
            <label for="seguro" class="col-md-12 control-label">{{trans('laboratorio.seguro')}}</label>
            <div class="col-md-12"> 
              <select id="seguro" name="seguro" class="form-control input-sm" onchange="subnivel();">
          ...
          <div class="form-group col-md-2 col-xs-6">
            <label for="nivel" class="col-md-12 control-label">{{trans('laboratorio.nivel')}}</label>
            <div class="col-md-12" id="js_subnivel"> 
              
            </div>
          </div>

          <div class="form-group col-md-2 col-xs-6">
            <label for="facturadas" class="col-md-12 control-label">{{trans('laboratorio.facturadas')}}</label>
            <div class="col-md-12"> 
              <select id="facturadas" name="facturadas" class="form-control input-sm" onchange="buscar();">
                <option @if($facturadas == "TODAS") selected @endif value="TODAS">{{trans('laboratorio.todas')}}</option>
                <option @if($facturadas == "FACTURADAS") selected @endif value="FACTURADAS">{{trans('laboratorio.facturadas')}}</option>
                <option @if($facturadas == "NO FACTURADAS") selected @endif value="NO FACTURADAS">{{trans('laboratorio.no_facturadas')}}</option>
              </select>
            </div>
          </div>
          ...
          <div class="form-group col-md-3 col-xs-6">
            <label for="nombres" class="col-md-12 control-label">{{trans('laboratorio.paciente')}}</label>
            <div class="col-md-12">
              <div class="input-group">
                <input value="@if($nombres!=''){{$nombres}}@endif" type="text" class="form-control input-sm" name="nombres" id="nombres" placeholder="{{trans('laboratorio.nombres_apellidos')}}" style="text-transform:uppercase;" onkeyup="javascript:this.value=this.value.toUpperCase();">
                <div class="input-group-addon">
          ...
          <div class="form-group col-md-3 col-xs-6">
            <label for="orden" class="col-md-12 control-label">Numero de Orden</label>
            <div class="col-md-12">
              <div class="input-group">
          ...
          <div class="form-group col-md-1 col-xs-2">
            <button style="margin: 5px" type="submit" class="btn btn-primary btn-xs" id="boton_buscar">
              <span class="glyphicon glyphicon-search" aria-hidden="true"></span>
            </button>
            <button style="margin: 5px" type="submit" class="btn btn-primary btn-xs" formaction="{{route('orden.reporte_index')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.reporte')}}
            </button>
          </div> 
          <div class="form-group col-md-1 col-xs-2">

            <button type="submit" class="btn btn-primary btn-xs" formaction="{{route('orden.reporte_mail')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.envio_mail')}}
            </button>
            
            <button type="submit" class="btn btn-primary btn-xs" formaction="{{route('orden.cotizaciones')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.cotizacion')}}
            <!--button type="submit" class="btn btn-primary btn-xs" id="boton_buscar">
              <span class="glyphicon glyphicon-search" aria-hidden="true"></span>
            </button-->
            <!-- <button style="margin: 5px" type="submit" class="btn btn-primary btn-xs" formaction="{{route('orden.reporte_mail')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.envio_mail')}}
            </button>
            <button style="margin: 5px" type="submit" class="btn btn-primary btn-xs" formaction="{{route('orden.cotizaciones')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.cotizacion')}}
          ...
          <div class="form-group col-md-1 col-xs-2">
            <button style="margin: 5px" type="submit" class="btn btn-success btn-xs" formaction="{{route('orden.reporte_detalle_covid')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.covid')}}
            </button>
            <button style="margin: 5px" type="submit" class="btn btn-primary btn-xs" formaction="{{route('orden.reporte_detalle')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.detalle_ordenes')}}
            </button>
          </div> 
         
          <div class="form-group col-md-2 col-xs-2">  
            
            <button style="margin: 5px" type="submit" class="btn btn-danger btn-xs" formaction="{{route('orden_labs.resultados_pendientes')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.examenes_pendientes')}}
            </button>

            <a style="margin: 5px" class="btn btn-success btn-xs" href="{{route('labscomisiones.comisiones')}}">{{trans('laboratorio.comisiones')}}</a>
          </div>   
               
          <div class="form-group col-md-2 col-xs-2">  
            <button style="margin: 5px" type="submit" class="btn btn-primary btn-xs" onclick="factura_agrupada();"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.factura_agrupada')}}
            </button>
            <button style="margin: 5px" type="submit" class="btn btn-primary btn-xs" formaction="{{route('orden.reporte_comercial')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> Reporte Comercial
            </button>
            <input type="hidden" class="form-control input-sm" name="tipo_reporte" id="tipo_reporte" value="comercial">
            <!-- <a class="btn btn-primary btn-xs" onclick="factura_agrupada();">{{trans('laboratorio.factura_agrupada')}}</a> -->
          </div>
          <div class="col-md-12">
                    &nbsp;
                </div>
          <div class="form-group col-md-2 col-xs-2">  
            ...
            <a style="margin: 5px" class="btn btn-warning" href="{{route('c_caja.index')}}">{{trans('laboratorio.cierre_caja')}}</a>
          </div>  


          <!-- <div class="form-group col-xs-4 col-xs-6" >
             <button type="submit" class="btn btn-primary btn-xs" formaction="{{ route('cierrecaja.conglomerada_cierre_laboratorio') }}" formtarget="_blank"><span class="glyphicon glyphicon-stats" aria-hidden="true"> Nuevo Reporte Cierre </button>
          </div> -->
      ...
      <div class="col-md-12" style="color: red;">
        <b>{{trans('laboratorio.pendientes_facturar')}}:</b>@foreach( $nofacturados as $nofacturado) @php $seguro = $seguros->find($nofacturado->id_seguro); @endphp {{$seguro->nombre}}: {{$nofacturado->cantidad}} &nbsp;@endforeach
      </div>
      <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
            ...
            <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info" style="font-size: 12px;overflow: none;">
              ...
              @foreach ($ordenes2 as $value) <!--FOR EACH PARA LOS PENDIENTES PUBLICOS ABAJO HAY OTRO-->
                ...
                @php 
                    ...
                    $vt_orden = Sis_medico\Examen_Orden::find($value->id); 
                  @endphp
                  <tr role="row" @if($user->id_tipo_usuario =='3') style="background-color: #ffe0cc;" @endif>
                    ...
                    <td>{{$value->toma_muestra}}@if($vt_orden->toma_muestras->count() > 0)<a data-toggle="modal" data-target="#log_muestra" data-remote="{{route('orden.log_toma_muestras',['id' => $value->id ])}}" class="btn btn-success btn-xs" ><i class="fa fa-plus"></i></a>@endif</td>
                    ...
                    <td>
                      @if($value->estado=='-1')
                        @if($value->stipo!='0')
                          <span class="label pull-right bg-red" style="font-size: 10px">{{trans('laboratorio.cotizacion')}}</span>
                        @else
                          <span class="label pull-right bg-red" style="font-size: 10px">{{trans('laboratorio.pendiente')}}</span>
                        @endif 
                      @else  
                        @if($value->estado_pago =='1')
                          <span class="label pull-right bg-green" style="font-size: 10px"> @if($value->pago_online=='1'){{trans('laboratorio.pago_enlinea')}} @else {{trans('laboratorio.pago')}} @endif</span>
                        @endif
                    ...
                    <td>
                      ...
                      <div class="col-md-12" style="padding-left: 0px">
                        <div class="btn-group">
                          <button type="button" class="btn btn-success btn-xs" onclick="descargar({{$value->id}});"><span style=" font-size: 10px">{{trans('laboratorio.resultados')}}</span></button>
              ...
              @foreach ($ordenes as $value)
                ...
                <tr role="row" @if($user->id_tipo_usuario =='3') style="background-color: #ffe0cc;" @endif>
                  ...
                  <td>{{$value->toma_muestra}}@if($vt_orden->toma_muestras->count() > 0)<a data-toggle="modal" data-target="#log_muestra" data-remote="{{route('orden.log_toma_muestras',['id' => $value->id ])}}" class="btn btn-success btn-xs" ><i class="fa fa-plus"></i></a>@endif</td>
                  <td style="font-size: 11px;">{{$value->papellido1}} @if($value->papellido2=='N/A'||$value->papellido2=='(N/A)') @else{{ $value->papellido2 }} @endif {{$value->pnombre1}} @if($value->pnombre2=='N/A'||$value->pnombre2=='(N/A)') @else{{ $value->pnombre2 }} @endif </td>
                  @php $nivel = Sis_medico\Nivel::find($value->id_nivel) @endphp
                  <td style="font-size: 11px;">{{$value->snombre}}-@if(!is_null($nivel)) {{$nivel->nombre}} @endif<br> {{$value->nombre_corto}}</td>
                  <td>
                    @if($value->estado=='-1')
                      @if($value->stipo!='0')
                        <span class="label bg-red" style="font-size: 10px">Cotiz.</span>
                      @else
                        <span class="label bg-red" style="font-size: 10px">{{$value->pre_post}}-Pendiente</span>
                      @endif 
                    @else  
                      @if($value->estado_pago =='1')
                        <span class="label bg-green" style="font-size: 10px">Pag. @if($value->pago_online=='1')Online @endif</span>
                      @endif
                      @if($value->pre_post!=null)
                        <span class="label bg-primary" style="font-size: 10px">{{$value->pre_post}}</span>
                      @endif
                    @endif
                  </td>
                  <td style="font-size: 11px;@if($user->id_tipo_usuario =='3') color: red; @endif">{{substr($value->cnombre1,0,1)}}{{$value->capellido1}}</td>
                  ...
                  <td>
                    ...
                    @if($value->stipo!='0')
                      @if($value->estado=='1')
                          ...
                          @else
                          @if($value->id_seguro == 4 or $value->id_seguro == 33)
                          <div class="col-md-4 ">
        ...
        <div class="row">
          <div class="col-sm-5">
            <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">{{trans('ecamilla.Mostrando')}} {{1+($ordenes->currentPage()-1)*$ordenes->perPage()}}  / @if(($ordenes->currentPage()*$ordenes->perPage())<$ordenes->total()){{($ordenes->currentPage()*$ordenes->perPage())}} @else {{$ordenes->total()}} @endif {{trans('ecamilla.de')}} {{$ordenes->total()}} {{trans('ecamilla.registros')}}</div>
          </div>
          <div class="col-sm-7">
            <div class="dataTables_paginate paging_simple_numbers" id="example2_paginate">
              {{ $ordenes->appends(Request::only(['fecha', 'fecha_hasta', 'nombres', 'seguro', 'id_nivel', 'facturadas']))->links()}}
            </div>
          </div>
        </div>
      </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function($){
   ...
   $('#modal_datosfacturas').on('hidden.bs.modal', function(){
    ...
    $('#log_muestra').on('hidden.bs.modal', function(){
                $(this).removeData('bs.modal');
            });         
  ...
  function subnivel(){
    var js_input_seguro = $("#seguro").val();
    $.ajax({
            type: 'post',
            url:"{{route('orden.obtener_convenio_seguro')}}",
            headers:{'X-CSRF-TOKEN':$('input[name=_token]').val()},
            datatype: 'json',
            data: { id_seguro: js_input_seguro, @if($id_nivel != null) id_nivel: '{{ $id_nivel }}' @endif },
            success: function(data){
                if(data!='no'){
                    $('#js_subnivel').empty().html(data);
                    //$('#div_nivel').removeClass('oculto');
                    $('#js_subnivel').show();
                }else{
                    //$('#div_nivel').addClass('oculto');
                    $('#js_subnivel').hide();
                    $('#js_subnivel').empty().html('');    
                }         
            },
            error: function(data){
