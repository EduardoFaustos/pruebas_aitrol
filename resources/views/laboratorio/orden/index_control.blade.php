...
<div class="modal fade" id="log_muestra" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-md-12">
          <h3 class="box-title"><b>{{trans('laboratorio.ingreso_resultados_laboratorio')}}</b></h3>
        </div>
  ...
  <div class="box-body">
      ...
      <form method="POST" action="{{route('orden.search_control')}}" id="lab_control">
        ...
        <div class="form-group col-md-2 col-xs-6">
            <label for="orden" class="col-md-12 control-label">{{trans('laboratorio.orden')}}</label>
            <div class="col-md-12">
        ...
        <button type="submit" class="btn btn-primary" id="boton_buscar">
              <span class="glyphicon glyphicon-search" aria-hidden="true"></span> {{trans('laboratorio.buscar')}}</button>
        </div>

        <div class="form-group col-md-2 col-xs-6">
          <button type="submit" class="btn btn-primary" formaction="{{route('orden.reporte_index')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.reporte')}}</button>
        </div>

        <button type="submit" class="btn btn-success" formaction="{{route('orden.reporte_detalle_covid')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.covid')}}</button>

        <button type="submit" class="btn btn-danger" formaction="{{route('orden_labs.resultados_pendientes')}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> {{trans('laboratorio.examenes_pendientes')}}</button>
      ...
      <div class="row">
        <div class="table-responsive col-md-12">
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info" style="font-size: 12px;">
            <thead>
              <tr role="row">
                <th width="5">{{trans('laboratorio.orden')}}</th>
                <th width="5">{{trans('laboratorio.fecha')}}</th>
                <th width="20">{{trans('laboratorio.nombres')}}</th>
                <th width="5">{{trans('laboratorio.seguro')}}</th>
                <th width="5">{{trans('laboratorio.tipo')}}</th>
                <th width="5">{{trans('laboratorio.creada')}}</th>
                <th width="10">{{trans('laboratorio.muestra')}}</th>
                <th width="10">{{trans('laboratorio.resultados')}}(%)</th>
                <th width="5">{{trans('laboratorio.biometria')}}</th>
                <th width="5">{{trans('laboratorio.bioquimica')}}</th>
                <th width="5">{{trans('laboratorio.manual')}}</th>
                <th width="20">{{trans('laboratorio.accion')}}</th>
              </tr>
            </thead>
            <tbody>
            @foreach ($ordenes as $value)
              <tr role="row" >
                ...
                <td>{{$value->toma_muestra}}@if($value->toma_muestras->count() > 0)<a data-toggle="modal" data-target="#log_muestra" data-remote="{{route('orden.log_toma_muestras',['id' => $value->id ])}}" class="btn btn-success btn-xs" ><i class="fa fa-plus"></i></a>@endif</td>
                ...
                <td>
                  @if($value->er_biometria=='2')
                    @php $texto = trans('laboratorio.listo'); $color = 'btn btn-success btn-xs'; @endphp
                  @elseif($value->er_biometria=='1')
                    @php $texto = trans('laboratorio.certificar'); $color = 'btn btn-warning btn-xs'; @endphp
                  @else
                    @php $texto = trans('laboratorio.pendiente'); $color = 'btn btn-danger btn-xs'; @endphp
                  @endif
                ...
                <td>
                  ...
                  @if(Auth::user()->id_tipo_usuario=='1')
                  <div class="col-md-2">
                    <a class="btn btn-block btn-success btn-xs" href="{{route('plantilla.orden_plantilla', ['id' => $value->id])}}"><i class="fa fa-file-powerpoint-o" aria-hidden="true"></i></a>
                  </div>

                  <div class="col-md-2">
                    <a class="btn btn-block btn-success btn-xs" href="{{route('laboratorio.plantilla.comparativo', ['id' => $value->id])}}"><i class="fa fa-braille" aria-hidden="true"></i></a>
                  </div>
                  @endif
                </td>
...
vartiempo = setInterval(function(){ pentax_semaforo(); }, 50000);
