...
<div class="modal fade fullscreen-modal" id="doctor" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  ...
  <div class="box-header">
    <div class="row">
        <div class="col-md-6">
          <h3 class="box-title">Listado de Registro AGRUPADORES DE EXAMENES</h3>
        </div>
        <div class="form-group col-md-3">
          <a class="btn btn-primary" href="{{ route('exa_agrupadores.create')}}"> Agregar</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{route('exa_agrupadores.search')}}">
        {{ csrf_field() }}
        <div class="form-group col-md-6">
          <label for="examen" class="col-md-2 control-label">Nombre Agrupador</label>
          <div class="col-md-10">
    ...
    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
      <div class="row">
        <div class="table-responsive col-md-12">
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info" style="font-size: 12px;">
            ...
            <tbody>
            @foreach ($examen_agrupador as $value)
              <tr role="row">
                <td>{{$value->id}}</td>
                <td>{{$value->nombre}}</td>
                ...
                <td><div class="form-group col-md-3">
                        ...
                        <a href="{{ route('exa_agrupadores.edit', ['id' => $value->id]) }}" class="btn btn-block btn-warning btn-xs" >
      ...
      <div class="row">
        <div class="col-sm-5">
          <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">Mostrando 1 al {{count($examen_agrupador)}} de {{$examen_agrupador->total()}} Registros</div>
        </div>
        <div class="col-sm-7">
          <div class="dataTables_paginate paging_simple_numbers" id="example2_paginate">
            {{ $examen_agrupador->appends(Request::only(['id', 'apellidos', 'nombres']))->links()}}
          </div>
        </div>
      </div>
    </div>
