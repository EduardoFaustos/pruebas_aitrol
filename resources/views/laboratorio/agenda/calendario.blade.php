        ...
        $('#calendar').fullCalendar({
            ...
            events : [
                 @foreach($agenda as $value)
                {
                  ...
                  title : '{{ $value->pnombre1}} @if($value->pnombre2 != "(N/A)"){{ $value->pnombre2}}@endif {{ $value->papellido1}}  @if($value->papellido2 != "(N/A)"){{ $value->papellido2}}@endif ({{ $value->nombre_seguro}}), TOMA DE MUESTRA | @if($value->estado_cita == 4) ASISTIÓ @elseif($value->estado_cita == 0) Por Confirmar @elseif($value->estado_cita == 1) Confirmado @elseif($value->estado_cita == 2) Confirmar Datos @endif | Cortesia: {{ $value->cortesia}} | {{$value->nombre_seguro}}',  
                  start : '{{ $value->fechaini }}',
                  end : '{{ $value->fechafin }}',
                  //url: '{{ route("agendalabs.laboratorio", ['id' => $value->id_paciente])}}',
                  //url: '{{ route('agenda.edit2', ['id' => $value->id, 'doctor' => '4444444444'])}}',
                  @if($value->estado_cita == 0)
                    color: 'black',
                  @else
                    color: '{{ $value->color}}', 
