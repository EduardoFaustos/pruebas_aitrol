...
.table-texto tfoot{
  ...
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item"><a href="../">{{trans('etodos.ProyecciónFinanciera')}}</a></li>
    </ol>
  </nav>
  <div class="box">
      ...
      <div class="box-body dobra">
        <form method="POST" id="reporte_master" action="{{ route('proyeccionfinanciera.resultado') }}" >
        {{ csrf_field() }}
      ...
      <div id="example2_wrapper" class="form-inline dt-bootstrap">
          ...
          <div class="col-md-12">
            <div class="box box-solid">
              ...
              <div class="box-body">
                ...
                <div class="col-md-4">
                  <h4 style="text-align: center;">{{trans('etodos.ProyecciónFinanciera')}}</h4>
        ...
        <div class="row">
          ...
          <div class="content">
            ...
            <table id="example2" class="table table-condensed table-texto" role="grid" aria-describedby="example2_info" style="font-size: 12px;">
              <thead>
                <tr class='well-dark'>
                  ...
                  <th width="10%" class="" style="text-align: right; " tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Codigo: activate to sort column ascending">{{trans('etodos.Ingresos')}}</th>
                  <th width="10%" class="" style="text-align: right; " tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Codigo: activate to sort column ascending">{{trans('etodos.Egresos')}}</th>
                  <th width="10%" class="" style="text-align: right; " tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Codigo: activate to sort column ascending">Var. {{trans('etodos.Ingresos')}}</th>
                  <th width="10%" class="" style="text-align: right; " tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Codigo: activate to sort column ascending">Var. {{trans('etodos.Egresos')}}</th>
              ...
              <tbody >
                ...
                @php
                  ...
                  $general = array();
                @endphp
                @foreach($fechagrupo as $key => $value)
                  @php
                    $cont = $cont+1;
                    $ingreso  = $ingresos[$value][0]['saldo'];
                    $gasto = $costos[$value][0]['saldo']+$gastos[$value][0]['saldo'];
                    $general[$value]['xy'] = $ingreso*$gasto;
                    //dd($general);
                    $general[$value]['x2'] = $ingreso*$ingreso;
                    $general[$value]['y2'] = $gasto*$gasto;
                  @endphp
                  <tr>
                    <td>{{$value}}</td>
                    <td>{{$ingreso}}</td>
                    ...
                    @if($key != 0)
                      @php
                        ...
                        $avtg = $avtg + $vag;
                      @endphp
                      <td>{{round($vai,4)*100}} %</td>
                      <td>{{round($vag,4)*100}} %</td>
                    @else
                      @php
                        $vai = 0;
                        $vi = $ingreso;
              ...
              <tfoot>
                <tr>
                    <td style="">{{trans('etodos.TOTAL')}}</td>
                    <td style="text-align: right; @if($vti < 0) color:red; @endif" >{{number_format($vti,2)}}</td>
                    <td style="text-align: right; @if($vtg < 0) color:red; @endif" >{{number_format($vtg,2)}}</td>
                    <td style="text-align: right; @if($avti < 0) color:red; @endif" >{{number_format(($avti/$cont),2)}}</td>
                    <td style="text-align: right; @if($avtg < 0) color:red; @endif" >{{number_format(($avtg/$cont),2)}}</td>
                    <td colspan="3">&nbsp;</td>
                </tr>
              </tfoot>
            </table>
          ...
          <div class="content">
            ...
            <table id="example2" class="table table-condensed" role="grid" aria-describedby="example2_info" style="font-size: 12px;">
              ...
              <tbody>
                ...
                @foreach($fechagrupo as $key => $value)
                    @php
                      $axy = $axy +$general[$value]['xy'];
                      $ax2 = $axy +$general[$value]['x2'];
                      $ay2 = $axy +$general[$value]['y2'];
                    @endphp
                    <tr>
                      <td>{{$value}}</td>
                      <td style="font-size: 10px;text-align: right; @if($general[$value]['xy'] < 0) color:red; @endif" >{{number_format($general[$value]['xy'],2)}}</td>
                      <td style="font-size: 10px;text-align: right; @if($general[$value]['x2'] < 0) color:red; @endif" >{{number_format($general[$value]['x2'],2)}}</td>
                      <td style="font-size: 10px;text-align: right; @if($general[$value]['y2'] < 0) color:red; @endif" >{{number_format($general[$value]['y2'],2)}}</td>
                    </tr>
                @endforeach
              </tbody>
              <tfoot>
                <tr>
                    ...
                    <td style="font-size: 10px;text-align: right; font-weight: bold; @if($axy < 0) color:red; @endif" >{{number_format($axy,2)}}</td>
                    <td style="font-size: 10px;text-align: right; font-weight: bold; @if($ax2 < 0) color:red; @endif" >{{number_format($ax2,2)}}</td>
                    <td style="font-size: 10px;text-align: right; font-weight: bold; @if($ay2 < 0) color:red; @endif" >{{number_format(($ay2),2)}}</td>
                </tr>
              </tfoot>
            </table>
          ...
          <div class="content" style="min-height: 100px;">
            ...
            <table id="example2" class="table table-condensed" role="grid" aria-describedby="example2_info" style="font-size: 12px;">
              ...
              <tbody>
                <tr>
                    <td style="font-size: 10px;font-weight: bold;">{{trans('etodos.PROMEDIO')}} X</td>
                    <td style=" ">{{number_format(($vti/$cont),2)}}</td>
                </tr>
                <tr>
                    <td style="font-size: 10px;font-weight: bold;">{{trans('etodos.PROMEDIO')}} Y</td>
                    <td style=" ">{{number_format(($vtg/$cont),2)}}</td>
                </tr>
              </tbody>
              </table>
              </div>
              ...
              <tbody>
                ...
                @endphp
                  <tr>
                    ...
                    @if ($anios==1)
                    @php $regresion=($value['costos']*$formulab)+$formulaa; $error=$value['gastos']-$regresion;  $aerror+=$error; @endphp
                    <td style="font-size: 10px;text-align: right; @if($value['costos'] < 0) color:red; @endif" >{{number_format($value['costos'],2)}}</td>
                    <td style="font-size: 10px;text-align: right; @if($value['vingresos'] < 0) color:red; @endif" >{{($value['vingresos'].' %')}}</td>
                    <td style="font-size: 10px;text-align: right; @if($regresion < 0) color:red; @endif" >{{number_format($regresion,2)}}</td>
                    <td style="font-size: 10px;text-align: right; @if($value['gastos'] < 0) color:red; @endif" >{{number_format($value['gastos'],2)}}</td>
                    ...
                    @php $acostos+=$value['aingresos']; $agastos+=$value['agastos']; $avcostos+=$value['avingresos']; $avgastos+=$value['avingresos']; $ultiing=$value['costos']; @endphp
                    @endforeach
                @php $index++;  @endphp
                @endforeach
          ...
          <div class="content">
            ...
            <table id="example2" class="table table-condensed" role="grid" aria-describedby="example2_info" style="font-size: 12px;">
              ...
              <tbody>
                ...
                @if ($cont==0)
                  @php
                  if($numanios!=0 && $avcostos!=0 ) {$inProyeccion=round($value['costos']+($value['costos']/($avcostos/$numanios)),2);} else {$inProyeccion=0;}
                  //$inProyeccion=($value['costos']+($value['costos']/($avcostos/$numanios)));
  ...
  <form method="POST" id="print_reporte_master" action="{{ route('proyeccionfinanciera.resultado') }}" target="_blank">
    {{ csrf_field() }}
