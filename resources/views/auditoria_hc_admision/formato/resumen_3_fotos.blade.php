				...
				@if($i < 3)
					...
					@endif
						...
						<td style="margin: 0;padding: 0 2px 0px 0px;">
							<div style="overflow: hidden; width: 252px; height: 252px; @if($j == 0){{'margin-left: -0px;'}}@elseif($j == 1) @if($imagenes[0]->seleccionado_recortada == 0){{'margin-left: -13px;'}}@else{{'margin-left: -116px;'}}@endif @else @if($imagenes[1]->seleccionado_recortada == 0){{'margin-left: -13px;'}}@else{{'margin-left: -116px;'}}@endif @endif  ">
								<img style="width: 355px; height: 248px;" src="{{base_path().'/storage/app/hc_ima/'.$value->nombre}}" hspace=12>
							</div>
						</td>
						@else
						<td style="margin: 0;padding: 0 2px 0px 0px;">
							<div style="overflow: hidden; width: 252px; height: 252px; @if($j == 0){{'margin-left: -0px;'}}@elseif($j == 1) @if($imagenes[0]->seleccionado_recortada == 0){{'margin-left: -13px;'}}@else{{'margin-left: -116px;'}}@endif @else @if($imagenes[1]->seleccionado_recortada == 0){{'margin-left: -13px;'}}@else{{'margin-left: -116px;'}}@endif @endif">
								<img style="width: 240px; height: 252px;position: absolute;" src="{{base_path().'/storage/app/hc_ima/'.$value->nombre}}" hspace=12>
							</div>
						</td>
						@endif
					@if($j >= 2)
