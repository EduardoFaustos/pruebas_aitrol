...
<head>
    ...
    #footer { position: fixed; left: 0px; bottom: -312px; right: 0px; height: 350px; font-size: 10px !important;}
			body{
				margin: 0;
				padding: 0;
			}
...
<body>
	...
	<div id="contenido">
		<table id="encabezado">
			...
			<tr>
				...
				<td style="font-size: 12px;"> @if(!is_null($protocolo->referido_por)) {{$protocolo->referido_por}} @else DR. CARLOS ROBLES MEDRANDA @endif</td>
		...
		<table style="margin-left: -23px;padding: 0;" >
			...
			@if($protocolo->auditoria_procedimiento->procedimiento_completo != "")
				@if($protocolo->auditoria_procedimiento->procedimiento_completo->grupo_procedimiento->imagen != "")
					@if($i < 8)
						@if($j == 0)
						<tr style="padding: 0;">
						@endif
						<td style="margin: 0;padding: 0 4px 0px 0px;"><img style="width: 179px; height: 179px;" src="{{base_path().'/storage/app/procedimiento_completo/'.$protocolo->auditoria_procedimiento->procedimiento_completo->grupo_procedimiento->imagen}}" hspace=12></td>
						@if($j >= 3)
						</tr>
						@endif
			...
			@else
				@php
					foreach($adicionales as $value2)
					{
						$procedimiento = \Sis_medico\Procedimiento::find($value2->id_procedimiento);

						$grupo = \Sis_medico\grupo_procedimiento::find($procedimiento->id_grupo_procedimiento);
						if(!is_null($grupo)){
							if(!is_null($grupo->imagen)){ @endphp
								@if($j == 0)
								<tr style="padding: 0;">
								@endif
								<td style="margin: 0;padding: 0 4px 0px 0px;"><img style="width: 179px; height: 179px;" src="{{base_path().'/storage/app/procedimiento_completo/'.$grupo->imagen}}" hspace=12></td>
								@if($j >= 3)
								</tr>
								@endif
								@php $i = $i+1;$j = $j+1; @endphp
				...
				@if($i < 200)
					...
					@endif
						...
						<td style="margin: 0;padding: 0 4px 0px 0px;">
							<div style="overflow: hidden; width: 180px; height: 179px; 
							@if($j != 4) 
							@if($j != 0)
							{{"margin-left: -72px;"}}
							@else{{"margin-left: 0;"}}
						...
						@else
						<td style="margin: 0;padding: 0 4px 0px 0px;">
							<div style="overflow: hidden; width: 250px; height: 179px; 
							@if($j != 4) 
							@if($j != 0)
							{{"margin-left: -72px;"}}
							@else{{"margin-left: 0;"}}
	...
	<div id="footer">
		...
		<br>
		<div style="width: 100%;">
			@if(!is_null($protocolo->auditoria_procedimiento->id_doctor_responsable))
				@php
					if(!is_null($protocolo->auditoria_procedimiento->id_doctor_responsable)){
			            $firma_2 =  \Sis_medico\Firma_Usuario::where('id_usuario', $protocolo->auditoria_procedimiento->id_doctor_responsable)->get();
			        }else{
			            $firma_2 = null;
			        }
			        $responsable = \Sis_medico\User::find($protocolo->auditoria_procedimiento->id_doctor_responsable);
			        //dd($responsable);
				@endphp
