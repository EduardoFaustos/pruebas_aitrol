      ...
      <div class="box">
        ...
        {{ csrf_field() }}
                ...
                <div class="form-group col-md-6{{ $errors->has('id_proveedor') ? ' has-error' : '' }}">
                        ...
                        <div class="col-md-8">
                          <select name="tipo_" class="form-control select2 " style="width: 100%;" required="" name="tipo_">
                              ...
                              <option @if("1"== $pedido->tipo) selected @endif value="1">Guia de Remision</option>
                              <option @if("2" == $pedido->tipo) selected @endif value="2">Factura Contra Entrega</option>
                              <option @if("3"== $pedido->tipo) selected @endif value="3">Factura</option>
                          </select>
                        </div>
                </div>
                <!-- Proveedor -->
          ...
          <div class="box-body">
            <div class="table-responsive col-md-12">
              ...
              <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                ...
                @foreach($cantidad_pedido as $detalle_pedido)
                  @php
                    if($detalle_pedido->iva == '1'){
                      $subtotal_12 = $subtotal_12 + ($detalle_pedido->precio * $detalle_pedido->cantidad);
                    }else{
                      $subtotal_0 = $subtotal_0 + ($detalle_pedido->precio * $detalle_pedido->cantidad);
                    }
                  @endphp
                @endforeach
                @php
                  $iva = $subtotal_12 * 0.12;
                  $total_final = ($subtotal_12*1.12) + $subtotal_0 -$pedido->descuento;
                @endphp
                ...
                @endphp
                <tr id="dato{{$i}}" >
                  <td>{{$detalle_pedido->codigo}} <input type='hidden' id='visibilidad{{$i}}' name='visibilidad{{$i}}' value='1' > <input type='hidden' id='iva{{$i}}' value='{{$detalle_pedido->iva}}' > <input type="hidden" name="id{{$i}}" value="{{$detalle_pedido->id_producto}}"> <input type="hidden" name="usos{{$i}}" value="{{$detalle_pedido->usos}}"> </td>
                  <td>{{$detalle_pedido->nombre_producto}}</td>
                  <td><input style="width: 60px" onkeyup ='total_calculo({{$i}})' @if($producto->despacho == 0) type="text" @else type="hidden" @endif name="cantidad{{$i}}" id="cantidad{{$i}}" value="{{$detalle_pedido->cantidad}}">  @if($producto->despacho == 1) 1 @endif </td>
                  ...
                  <td><div class='form-group  lote_error{{$i}}'><input type="text" name="lote{{$i}}" id="lote{{$i}}" value="{{$detalle_pedido->lote}}"></div></td>
                  <td>{{$detalle_pedido->registro_sanitario}}</td>
                  <td><input type="date" name="fecha_vencimiento{{$i}}" id="f_venci" value="{{$detalle_pedido->fecha_vencimiento}}"></td>
                  <td><input style="width: 60px" onkeyup ='total_calculo({{$i}})' value='{{$detalle_pedido->descuentop}}' id='descuento{{$i}}' type='text' name='descuento{{$i}}'  > <input type='hidden' name="descuentof{{$i}}" id="descuentof{{$i}}" value="{{$detalle_pedido->descuento}}" </td>
                  <td><input style="width: 60px" onkeyup ='total_calculo({{$i}})' value='{{$detalle_pedido->precio}}' id='precio{{$i}}' type='number' name='precio{{$i}}' ></td>
                  <td> <span id='precio_final{{$i}}' name='precio_final{{$i}}'>{{$detalle_pedido->precio * $detalle_pedido->cantidad }}</span>  </td>
                  ...
                  <td><button type='button' onclick='eliminardato({{$i}})' class='btn btn-warning btn-margin'>Eliminar</button></td>
                  @php $i++; @endphp
                </tr>
                @endforeach
                </tbody>
                ...
                <tfoot>
                  <tr>
                    <td colspan="9"></td>
                    <td>Subtotal 12%:</td>
                    <td><input type="hidden" name="subtotal_12" id="subtotal_12" value="{{$subtotal_12}}"> <input style="width: 55px;" type="text" readonly id="subtotal_12_1" value="{{$subtotal_12}}"></td>
                    <td></td>
                  </tr>
                  <tr>
                    <td colspan="9"></td>
                    <td>Subtotal 0%:</td>
                    <td><input type="hidden" name="subtotal_0" id="subtotal_0" value="{{$subtotal_0}}"> <input style="width: 55px;" type="text" readonly id="subtotal_0_1" value="{{$subtotal_0}}"></td>
                    <td></td>
                  </tr>
                  <tr>
                    <td colspan="9"></td>
                    <td>Descuento :</td>
                    <td><input type="hidden" name="descuent" id="descuent" value="{{$pedido->descuento}}"> <input style="width: 55px;" type="text" name="descuentx" readonly id="descuentx" value="{{$pedido->descuento}}"></td>
                    <td></td>
                  </tr>
                  <tr>
                    <td colspan="9"></td>
                    <td>IVA:</td>
                    <td><input type="hidden" name="iva" id="iva" value="{{$iva}}"> <input style="width: 55px;" type="text" readonly id="iva_1" value="{{$iva}}"></td>
                    <td></td>
                  </tr>
                  <tr>
                    <td colspan="9"></td>
                    <td>Total:</td>
                    <td><input type="hidden" name="total" id="total" value="{{$total_final}}"> <input style="width: 55px;" type="text" readonly id="total_1" value="{{$total_final}}" ></td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
          ...
          <div class="box-footer" style="text-align: center;">
            ...
            @if($pedido->tipo!=3)
            <button type="submit" class="btn btn-success btn-gray" formaction="{{route('ingreso.generar_facturas')}}" data-loading-text="<i class='fa fa-spinner fa-spin '></i> Procesando Informacion"> <i class="fa fa-save"></i> Generar Factura</button>
            @endif
          </div>
        </form>
      </div>
