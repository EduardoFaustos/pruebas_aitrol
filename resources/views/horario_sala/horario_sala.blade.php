        ...
        <div class="col-md-12">
          <div class="box box-default @if(session()->get('error')  ) @else collapsed-box @endif  " >
            <div class="box-header with-border">
              <h4>{{trans('etodos.AgregarDíaLaborableporFecha')}}</h4>
            ...
            <div class="box-body">
              <form class="form-horizontal" role="form" method="POST" action="{{ route('horario_sala.unico2') }}">
                <input id="id_sala1" type="hidden" name="id_sala1" value="{{$id}}" >
                {{ csrf_field() }}
                         
                        <div class="form-group col-md-6 {{ session()->get('error') ? ' has-error' : '' }} {{ $errors->has('id_sala1') ? ' has-error' : '' }} "  >
                            ...
                            <div class="col-md-7">
                                <div class="input-group date">
                                   ...
                                   @if (session()->get('error'))
                                    ...
                                    @endif
                                    @if ($errors->has('id_sala1'))
                                    <span class="help-block">
                                        <strong>{{ $errors->first('id_sala1') }}</strong>
                                    </span>
                                    @endif
                            </div>
                        </div>
                        ...
                        <div class="form-group col-md-6 {{ session()->get('error') ? ' has-error' : '' }}  {{ $errors->has('id_sala1') ? ' has-error' : '' }}">
                            <label class="col-md-4 control-label">{{trans('etodos.Fin')}}</label>
                            <div class="col-md-7">
                                ...
                                <div class="input-group date">
                                    ...
                                    <input autocomplete="off" type="text" value="@if(session()->get('inicio')) {{ session()->get('fin')}} @endif" name="fin" class="form-control" id="fin" required>
                                </div>
                                @if (session()->get('error'))
                                    <span class="help-block">
                                      <strong>{{ session()->get('error') }}</strong>
        ...
        <div class="col-md-12">
            <div class="box box-primary ">
                <div class="box-body with-border ">
                  ...
                  {!! Form::open(['route' => ['horario_sala.actualizar'], 'method'=>'POST' ])!!} 
                  {!! Form::close() !!}    
                </div>  
            </div>
        </div>
        ...
        $('#calendar').fullCalendar({
            ...
            events: [
              ...
              { 
                ...
                editable: true,
                id: '{{$value->id}}',
                color: '@if($value->tipo == 1) #6666FF @endif @if($value->tipo == 2) #FF00BF @endif @if($value->tipo == 3) #00e600 @endif',
                overlap:false,
                extra: '0',
              },
              @endforeach 
            ...
            eventResize: function(event, start, end) {  
               ...
               var url = "{{ route('horario_sala.actualizar') }}/"+id+"/"+start+"/"+end+'/'+extra;
            ...
            select: function(start, end, jsEvent) {  // click on empty time slot
               ...
               $("#agregarHorario2").load('{{route("horario_sala.agregar_modal2")}}/'+start+'/'+end+'/{{$id}}');
            ...
            eventDrop: function(event, delta, start, end){ // event drag and drop
               ...
               var end = event.end;
               var extra = event.extra;
               var url = "{{ route('horario_sala.actualizar2') }}/"+id+"/"+start+"/"+end+"/"+extra;
               $.get(url, function(result){
                  alert(result);
               });
            },
            eventClick: function(event) {
              ...
              if (mensaje) {
                if(event.extra == "0"){
                  ...
                  var id = event.id;
                  var end = event.end;
                  var url = "{{ route('horario_sala.eliminar') }}/"+id;
                  $.get(url, function(result){
                     alert(result);
                  });
                ...
                if(event.extra == "1"){
                  var mensaje2 = confirm("Al eliminar este horario se procedera a reagendar todos los procedimientos y consultas del mismo ¿Esta Seguro?");
                  if (mensaje2) {
                    var start = event.start;
                    var id = event.id;
                    var end = event.end;
                    var url = "{{ route('horario_sala.eliminar2') }}/"+id;
                    $.get(url, function(result){
