@php 
$fecha_cortada = substr ($fecha->fechaini,0,11);
@endphp
<style>
  ...
  <div class="modal-content">
    ...
    <input type="hidden" value="{{$empresa->id}}" name="empresacheck" > 
      ...
      <h3 style="text-align: center;color:white;font-size:30pxt;font-weight:bold" class="modal-title"> PREPARACIÓN </h3>
    </div>
    <form method="post"  action="{{route('descargar',[$empresa->id])}}">
    {{csrf_field()}}
    <input type="hidden" name="id" value="{{$paciente->id}}">
      ...
      <div style="margin-top: 2px;" class="col-md-12">
        <div class="form-row">
          ...
          <div class="form-group col-md-4">
            <label for="nombre_procedimiento" class="col-form-label-sm">Fecha</label>
                <div class="input-group date" style="width:80%">
                    <div class="input-group-addon">
                        <i class="fa fa-calendar"></i>
                    </div>
                    <input type="text" value="@if(old('inicio')!=''){{old('inicio')}}@else{{$fecha_cortada}}@endif" name="inicio" class="form-control pull-right input-sm" id="inicio" >
                </div>
            </select>
          </div>
        </div>
