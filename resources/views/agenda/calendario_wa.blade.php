...
?>
            ...
            events : [
                    ...
                    @endphp 
                {
                  ...
                  title : '{{ $value->papellido1}} @if($value->papellido2 != "(N/A)"){{ $value->papellido2}}@endif {{ $value->pnombre1}} @if($value->pnombre2 != "(N/A)"){{ $value->pnombre2}}@endif @if($value->paciente_dr=="1") /PART @endif | CONSULTA @if($value->vip=="1") /VIP @endif | @if($versuspendidas==0) @if($value->estado_cita == 0) Por Confirmar @endif @if($value->estado_cita == 1)  Confirmada @endif @if($value->estado_cita == 2) @if($value->estado==1)  Completar Datos @else  Por Reagendar @endif @endif @if($value->estado_cita == 4)  ASISTIÓ @endif| @endif @if($value->tipo_cita == 0) Primera vez @endif @if($value->tipo_cita == 1) Consecutivo @endif | Agenda: {{$value->uapellido1}} | Modif: {{$value->umapellido1}} | Seguro: @if($value->estado_cita!='4'){{ $value->nombre_seguro}} @else {{$hc_seguro_nombre}}@endif | Cort.: {{ $value->cortesia}} @if($value->omni=='SI') | OMNI @endif @if($value->ciudad != null) | Ciu.Proc: {{$value->ciudad}} @endif @if(!empty($hc_procedimientos->hora_fin)) |**ATENDIDO** | @endif @if($id == "1314490929" && $receta_ok)|RECETA OK @endif |Teleconsulta: @if($value->tc=='1' )SI @else NO @endif',
                  @endif
                  start : '{{ $value->fechaini }}',
                  end : '{{ $value->fechafin }}',
                  @if($fpermiso)
